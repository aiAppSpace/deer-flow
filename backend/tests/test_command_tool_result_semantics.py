...
def test_setup_agent_empty_soul_receipt_is_error():
    ...
    # This producer *does* set status="error", the way upstream's own
    # `update_agent`, `batch_task` and `review_skill_package` do; the contrast is
    # `test_command_error_receipt_is_error_not_success` above, where the producer
    # does not. Either way the classification the rest of the system reads is
    # `_meta` and the receipt, which the two asserts below pin.

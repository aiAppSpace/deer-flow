...
def test_prod_default_still_builds_via_preview() -> None:
    ...
    # This fork runs two frontends, so the variable is REACT_FRONTEND_CMD; the
    # contract being pinned (which command each branch picks) is unchanged.
    assert re.search(
        r"elif\s+\$SKIP_FRONTEND_BUILD;\s+then.*?REACT_FRONTEND_CMD=.*?run start.*?\nelse\n\s*REACT_FRONTEND_CMD=.*?run preview",
        serve,
        re.S,

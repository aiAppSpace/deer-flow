...
- `write_file` enforces the configured single-call size cap and remains the small-file/edit tool. Long generated deliverables use the staged artifact protocol: `begin_artifact_write` initializes hidden staging files, ordered `append_artifact_chunk` calls add small payloads, and `finalize_artifact_write` validates then writes the final target. Completed `.html`/`.htm` artifacts must pass complete-document validation and local render-resource existence checks on every write path.
...
- Scheduled background runs are intentionally non-interactive: they execute through the normal run lifecycle, but the lead-agent toolset excludes `ask_clarification` when `context.non_interactive=true`. The key is honored only for internally-authenticated callers (the scheduler launch path); client-supplied `context.non_interactive` is dropped. The scheduler service itself is gated by `config.yaml -> scheduler.enabled`.
...
explicit flag, `DEERFLOW_SETTINGS_TEST_MEMORY_BACKEND=deermem|noop` selects one
checked-in manager through the normal config/factory path, while
`DEERFLOW_SETTINGS_TEST_HOME_MARKER` may publish only the current temporary
home path so acceptance can corrupt and restore its isolated `memory.json`.
Auth, CSRF, per-user manager/storage, Skills and MCP routers, atomic config

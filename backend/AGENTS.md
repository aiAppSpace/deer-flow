...
`scripts/run_replay_gateway.py` mounts two acceptance-only seeds, each behind its
own explicit flag: `DEERFLOW_ENABLE_SETTINGS_TEST_SEED=1`
(`tests/settings_e2e_fixture.py` — ephemeral operator-owned Skills/MCP inputs, plus
`DEERFLOW_SETTINGS_TEST_MEMORY_BACKEND` / `..._HOME_MARKER`) and
`DEERFLOW_ENABLE_TEST_SEED=1` (`tests/seed_runs_router.py` — seeded runs carrying a
`workspace_changes` payload written through the normal `run_event_store`).
**Auth, CSRF, per-user managers, the Skills/MCP routers, atomic config writes and
secret masking stay on the production path in both** — never add a test-only settings
router, never mount either seeder in the production app, and never replace those read
boundaries with fixture routes. Details live in the two fixture files.

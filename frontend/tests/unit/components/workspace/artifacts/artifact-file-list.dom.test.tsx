...
function renderList(
  ...
  const component = (
    ...
    <QueryClientProvider client={queryClient}>
      ...
      >
        {/*
          `ArtifactFileList` reads `isMock` from the thread context so a
          /showcase visitor gets the /mock/api download route (see the
          "download href" cases at the bottom of this file). These archive
          cases only need a real (non-mock) thread, but the provider has to be
          here — `useThread` throws outside one.
        */}
...
/*
  这一组与上面那组「zip 归档」互不相干，是本仓在 #5117 之前就有的回归守卫，
  合并上游时按原样接回来（上游的 renderList 不包 ThreadContext，所以自带一个）。

  回归内容：这份列表曾经忘了转发 mock-thread 标志，而
  artifact-file-detail.tsx 每个 URL 都转发。在 /showcase/<thread_id> 上，
  演示用的 artifact 由 /mock/api 路由处理器提供，普通的 /api/threads/... 链接
  指向要鉴权的 Gateway 路由，匿名访客读不到。
*/
describe("ArtifactFileList download href", () => {
  function renderWithThread(isMock: boolean) {
    const queryClient = new QueryClient({
      defaultOptions: { queries: { retry: false } },
    });
    return render(

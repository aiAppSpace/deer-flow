# React → Vue 平替：挂账总清单（截至 wave 144，2026-09-07）

这份文件回答一个问题：**「还欠什么」。** 逐条给状态，不给散文。
深度背景在 `vue-parity-handoff.md`，踩坑线索在 Claude 记忆 `deerflow-parity-harness-plan`。

> **判据提醒**：台账现在是 **219 行 / 89 个取样点**（wave 144）。
>
> **wave 144 新挂一笔、下一轮就修**：技能设置页那一屏第一次进取样面，量出
> **`TabsList`/`TabsTrigger` 的 variant 体系本仓整套没有**（上游 `default`/`line` 两档，
> 靠 `group/tabs-list` 联动；本仓另写了一套、连 `variant` prop 都没有），
> 外加头部顺序不同（上游「标签左、创建键右」，本仓创建键单独一行在上）。
> 14 行 × 两种语言，逐条读数在交接文档 wave 144。
>

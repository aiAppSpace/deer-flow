...
> | E | **推送节奏决定 CI 反馈延迟** | 上面那条的真正变量。两次推送之间攒了 **360 个提交**，等于把 CI 反馈推迟了 6 天——而 CI 是**唯一**能抓「只在 Linux 上成立」的失败的地方（字体度量、容器环境、后端依赖，本机一个都复现不了）。**这是用户的决定，不是我能单方面改的**：现行硬规则是「要推就先问用户」。可选做法：每轮收工推一次，用小增量换早反馈。**挂在这里等用户定。** |>
> ### 三、这一轮的两条方法教训
>

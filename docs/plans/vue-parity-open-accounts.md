...
> | # | 账 | 现状 |
> | - | -- | ---- |
> | D | **「批不超过 4 轮」这条规则没有机器在守** | 规则写在冷启动文档的节奏块里，而 `e2e-mock` 上一次真跑是**第十三轮**——到第二十轮隔了 **7 轮**，将近限额两倍，CI 上那条红因此躺了 6 天没人看见。**又一条「写下来了、没被执行」的规则**（同管道退出码那条）。可行做法：签入 `baseline/gate-runs.json` 记录每条全套门禁上次实跑的提交，守卫比 `git rev-list --count <那个提交>..HEAD`，超阈值就红。**要单独一轮做足负向验证。** |
>
> ### 三、这一轮的两条方法教训
>

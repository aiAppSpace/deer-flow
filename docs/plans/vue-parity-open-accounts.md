# React → Vue 平替：挂账总清单（截至 2026-09-15 第十九轮）
...
> | # | 账 | 现状 |
> | - | -- | ---- |
> | A | **收起态的侧栏整个不在取样面** | 上游收起靠 CSS（节点还在：分组标题 `-mt-8 opacity-0`、菜单动作 `hidden`、菜单键 `size-8! p-2!`），本仓靠 `v-if="sidebarExpanded"`（**节点删掉**）。`opacity-0` 不移出可访问性树，所以**上游收起后仍朗读分组标题、本仓不会**——很可能是上游缺陷，动就得两边同改。**没有任何场景会收起侧栏**，改完没有读数可验。起手式：先确认 `SidebarTrigger` 在 workspace 外壳里渲染得出来、点得到。 |
> | B | **CI 那条修复只验到「照抄了绿着的兄弟 job」** | 本地没法跑 CI；结论要靠推送验证，而**推送要先问用户**。门禁已经钉住两个 job 的装配一致，所以退化会红；但「补上之后 `make e2e-backend` 在 CI 上真的绿」尚未观测到。 |
>
> ### 三、这一轮的三条方法教训
>

...
| 6b    | **移动端抽屉的模态做法不同**（台账 30 行） | **wave 147 新挂，下一轮修**                  | `thread-list-pin#mobile-drawer` 是移动端侧栏第一次进取样面。27 行 `ariaOnlyVue`（抽屉背后的整页）+ `order` / `tabOrder` / `tabbablesOnlyReact` 各 1 行，同一处根因：上游是 Radix `Sheet`，它给兄弟节点打 `aria-hidden`；本仓是手写的 `role="dialog" aria-modal="true"` + 手写 Tab 陷阱。**两种都是正确做法**，而 Playwright 的快照认 `aria-hidden`、不认 `aria-modal`。**键盘那一半两边等价，这是量出来的**：`tabbablesOnlyVue` 为空，本仓的焦点陷阱确实把 Tab 关在抽屉里。**下一步**：把抽屉换成 reka 的 `DialogRoot modal`（那一档会自己打 `aria-hidden`），这 30 行应当一起消失；剩 3 行是 `Sheet` 自带的 sr-only 标题与说明，按上游补或声明。**别顺手删 `aria-modal`**——换 primitive 是为了同时拿到 `aria-hidden`，不是因为 `aria-modal` 错。另有 2 行 `requestsOnlyReact`（`GET /api/features`、`POST /api/langgraph/threads/search`）**成因未查明，只记读数**，下一轮连模态一起查。                                                             |

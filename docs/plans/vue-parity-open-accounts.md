...
>
> ### 三之二、那 9 行为什么飘：一条**尚未验证**的假设
>
> 飘的那些全是 `background`（alpha 落在 230 / 102）与 `hit`（点上去命中谁），
> **两样都由鼠标位置决定**。Playwright 的指针停在上一步操作留下的地方，
> 而两个应用的布局本来就差 4px（A 组）——同一组点击坐标，在一边落在按钮上、
> 另一边落在按钮外，于是一边是 hover 态、另一边不是。
>
> **这是假设，不是结论**（本轮已经因为把假设当结论栽了三次）。
> 验的办法：取样前把指针移到中性位置，然后**连着量两次以上**看那 9 行还飘不飘。
> 用 `parity_only` 定点复量 `integrations#permission-request`，一次约 4 分钟。
>

...
>
> ⚠ **「现在」这一列是第三十五轮那一刻的值，不是今天的值**（2026-09-23 订正）：
...
> **第二十三轮收工时是 143 唯一行 / 163 多重集 / 142 个场景-维度 / 33 条不同的差异**
...
### ⚠ 收尾时 CI 红了一次——**是新门禁自己抓到的，而且抓到的是真账**

第五十六轮那次推送的 `frontend-vue parity` 在 Linux 上 **211 passed / 1 failed**，
红的正是这一轮新加的 `viewport-fit.spec.ts`（vue 那条）：

```
thread-list-pin#mobile-drawer[200%]: menu"Export as Markdown…" x=[0,384] vw=375
```

**本机（macOS）同一处量到的是 344，正好装得下。** 内容一模一样——
`Export as Markdown` 在 **Linux 的字体回退里就是更宽**，于是同一颗子菜单
344 → 384，在 375 的屏幕上溢出 9px。

> **判词：「在我这台机器上装得下」不是「装得下」。**
> 内容撑开的盒子如果只是**刚好**够，那它靠的是字体度量的运气；
> 换个平台、换个字体回退就破。**要么给它一个约束，要么它就是个定时炸弹。**

改法：给两边的 `DropdownMenuSubContent` 加 **`max-w-[calc(100vw-1rem)]`**。

⚠ **为什么是视口而不是 `available-width`**（第五十六轮试过并撤回的那个）：

```
available-width   上游 174 / 本仓 375 —— radix 从翻转那侧量，**基线字号就咬**，
                  把上游子菜单夹到 129px、条目换行；为修 200% 弄坏基线
100vw - 1rem      基线 359px（内容 181px，**不咬**）· 200% 343px（**才咬**）
```

改后本机实测（两应用逐字相同）：

```
基线   w=181  maxW=359px  不咬 —— **基线一格没动**
200%   w=343  maxW=343px  咬住 —— 本仓 x=[32,375] 完整在视口内
```

上游那两行仍在 `KNOWN_OUTSIDE`（它的问题是**位置**不是宽度），读数已同步为
`x=[201,544]`。条目在变窄时**换行不裁切**（Item 基类没有 `whitespace-nowrap`，
第五十六轮实测夹窄时条目高 32→52，是换行）。

> **这条门禁上线第一跑就在 CI 上抓到一条本机看不见的真账**——
> 它守的那一类（内容撑开的浮层贴着视口边）恰恰是**最容易被平台差异掀翻**的一类。

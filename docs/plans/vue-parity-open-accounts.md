...
>
> **wave 145 新挂一笔（第 7 条）**：**取样面看不见伪元素**，而 `line` 档的选中态
> 全靠 `::after` 那根下划线——整个 variant 的视觉主体不在任何一档的量程里。
>

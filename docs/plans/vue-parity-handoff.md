...
> - 对照台账 **137 唯一行 / 131 个场景-维度**（数 `baseline/parity-diff.json` 的
>   `entries`；量法写在冷启动文档里）；
> - **「台账还剩多少行」已经不能当坐标系**：137 行里 **95 行**是
>   `div[scroll-area-viewport]` 那一笔判过的账（含它在 `tabOrder` 上的投影）、
>   **12 行**是请求层那几条（`retry` 一族），剩下 30 行逐条有判词，
>   而且 2026-09-11 / 09-12 两轮都**主动把台账做大了**（新开 `dark` / `mobile` 维度、
>   给 diff 三档挂锚点）——**窄屏那个开着的工单 2026-09-12 已结清**。

...
> - 对照台账 **154 唯一行 / 133 个场景-维度**（数 `baseline/parity-diff.json` 的
>   `entries`；量法写在冷启动文档里）；
> - **「台账还剩多少行」已经不能当坐标系**：154 行里 **99 行**是
>   `div[scroll-area-viewport]` 那一笔判过的账（含它在 `tabOrder` 上的投影）、
>   **12 行**是请求层那几条（`retry` 一族），剩下 43 行逐条有判词，
>   而且第三、四、五轮**都主动把台账做大了**（新开 `dark` / `mobile` 维度、

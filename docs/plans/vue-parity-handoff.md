...
> - 对照台账 **0 唯一行 / 0 多重集 / 165 个场景-维度**（数 `baseline/parity-diff.json` 的
...
>   **0 条不同的差异**（2026-09-17 第三十六轮 accept 实测，同样有门禁守着）。
>   逐组清单见 `vue-parity-open-accounts.md` 的「零、全面审查」节；
>   那一节点名的账 F / G **第二十三轮已结清**，现状看该文件的第二十三轮条目；
> - **签入基线是 0 行，但那是 macOS 的读数。** 第三十七轮把 `e2e-parity` 接进
>   CI，**第一次运行就红**：同一棵树在 ubuntu-latest 上量出 16 行
>   （run 35143922501）。所以「清零」的边界是三重的——当前尺子、当前取样面、
>   **当前这台笔记本**。逐组读数与已判/未判写在
>   `vue-parity-open-accounts.md` 的第三十七轮条目；那 16 行里已判的只有一组
>   （取样器不等动画，本轮修了根因）。
> - **`frontend-vue parity` 这条工作流现在是红的，这是有意的**：它红着说明它在量
>   东西。不许用「重录基线」或「放宽几何容差」让它变绿。
> - **挂账总清单 `vue-parity-open-accounts.md` 的第一节里，4/5/6 三条的结论
>   在 2026-09-11 被推翻或部分推翻**，那一节已就地标注；**以该文件顶部的
>   2026-09-11 条目为准**。
>
> 下面这一段「截至 wave 202」是 2026-09-09 的快照，**数字与结论都已过期**，
> 留着是为了能追溯历史。

## 上一轮（2026-09-16 第三十三轮）做了什么

**族 D 还清：上游把一个列表塌成了一条。投影 36 → 23。**

- 后端 `GET /channels/connections` 返回列表、`DELETE /connections/{id}` 早就有，
  上游却在渲染前 `connectionByProvider` 塌成一条——**第二个账号看不见也解不掉**。
- **改的是上游**：列表化 + 每账号一行 + 逐账号断开 + 主操作键两分支共用
  （连上之后原本没有入口再加账号）+ 两条词条两个语言。
- 本仓只改了一处容器间距（`space-y-3` → `gap-4`，照上游）。
- **中间走错过一步**：以为描述里的 `Connected as` 是本仓多的、删掉，读数 10 → 22
  ——「哪一边多了」要看差异档的方向（`ariaOnlyVue` / `ariaOnlyReact`），
  凭印象删一句会把一处对齐改成两处不对齐。

## 上一轮（2026-09-16 第三十二轮）做了什么

**族 B 的大头还清：重试策略，两边都不对，两边同改。投影 69 → 36。**

- 三条读数（`integrations#load-failed` 12 / `scheduled-tasks#load-failed` 12 /
  `artifact-batched-stream#preview-failed` 9）是**同一个根因**：
  上游 `new QueryClient()` 吃 TanStack 默认 `retry: 3`（不分错误码），
  本仓写死 `retry: false`（连断网也不重试）。
- **新判据「只重试传输层失败」**（`fetch` 连接层失败抛原生 `TypeError`），
  `isRetryableTransportError` 两个应用逐字同一份，次数取 TanStack 自己的 3。
- **这正是 wave 128 挂账时写下的翻案判据**——挂账时写清翻案条件，是会被真的兑现的。
- **没做更细的「按状态码分流」**：上游没有带 `status` 的错误类型，
  加一套是贯穿式重构，越过「`frontend/` 只做小改」那条边界。翻案判据写在挂账清单。

## 上一轮（2026-09-16 第三十一轮）做了什么

**按「完全一致」这条最终判据还的第一笔账：族 A 清零。**

- **投影 170 → 69，不同的差异 35 → 31，`div[scroll-area-viewport]` 一族 101 → 0。**
- **根因不在 primitive，在「用在哪」**：两边的 `ScrollArea` 都给 viewport 加了
  `tabindex`，差的是上游 `ai-elements/suggestion.tsx` 的 `Suggestions`
  把建议行包在 ScrollArea 里，**本仓整层没有**。
  光看差集会绕进「两边都写了 tabindex，差异从哪来」的死胡同——
  这一轮把**完整可 tab 序列**做成了 `PARITY_ONLY` 的常驻探针，
  读数一眼看清：`React 35 个 / Vue 34 个`。
- **做法是补一层真组件**（新增 L2 `ui/suggestion/`），不是给那个元素硬加 `tabindex`。
  顺带把 chip 类串从两份本地常量收敛成单一出处，追问行那一段也逐条对齐了上游
  （两层容器、加载态与建议行互斥、关闭键在 `Suggestions` 里面、`py-1.5`、`cursor-pointer`）。
- **字面对齐被台账当场否掉的一次**：`CommandInput` 按字面对齐之后
  `parity-accept` 拒写，报出 `role:dialog[Model Selector] height Δ-13.1` 等六行几何，
  **而改之前那一屏几何是全对的**（底层 Reka `ListboxFilter` vs cmdk `Input` 不同构）。
  回退字面对齐，把实测读数写进判词 + `DECLARED` + 翻案判据。
  **判据取渲染一致而不是类串一致。**
- **沿途修好一把坏尺子**：`primitive-base-classes` 的提取器用非贪婪正则，
  遇到文件里更早的 `cn(..., props.xxx)` 会跨行吞错段落，
  **让整个组件从比对集合里静默消失**。换成配平括号扫描。
  `followup-chip-guards` 的锚点也从「相邻属性」改成「按语义定位 + 恰好一处」。

## 上一轮（2026-09-16 第三十轮）做了什么

**退役一条没量过前提的队列项，然后给尺子补上「天生看不见的第②类」。**

- **账 C 那条线退役**：队列第 1 条写着「还有多少本仓用原生 `title`、上游用 tooltip」
  ——**前提不成立**。三次读数：两边用量 **80 vs 72**（剥注释后）；
  按文件配对后仅有的两处「本仓有上游无」逐个查都对得上
  （run duration 上游在 `run-duration.tsx:53` 用同一个 `title`；
  CodeBlock 照的是 streamdown 自己的标记）；18 处 icon-only 的 `title`
  按 HTML 规范本来就是可访问名的兜底，台账也报不出差异。
  第二十四轮修的那四颗侧栏键是**具体一处**，不是可外推的规律。
- **第三扇维度窗，第三次零新差异**（`subtask-card` 补 dark）。
  连起来：第二十七轮两扇、第三十轮一扇，**连续三扇零**。这条线的单位产出
  已经量出来是低的，队列排序要照读数改，不照记忆改。
- **正题：请求体进取样面**（`ParityCapture.requestBodies` + `DiffEntry.requestBodies`）。
  归一化**只有两条规则**（对象按 key 排序、客户端生成的 UUID 抹掉），
  时间戳一律不抹（硬规则 2）；解析不了 JSON 的体原样留着。
  **只比两边都发过的键**，而且按「不同的体」的集合比——多重集写法第一跑就撞上
  「同一处差异两份投影」（`POST /api/threads/search` 体逐字相同、只是 React 多发一次，
  而那已经由 `requestsOnlyReact` 报过）。
- **第一跑就报了三行此前任何一档都看不见的差异**，逐条判词见挂账清单：
  Vue 建线程多带 `assistant_id`（保留本仓）、`stream_mode` 多一个 `values`
  （早就判过的一条，**被这把新尺子独立重量了出来**——它可信的旁证）、
  `stream_resumable: false`（保留本仓，后端显式接受它）。
  外加一条**挂着**：`context.thread_id` 两边不同，下一轮查。
- 配套两条「0 要算出来」：形状断言 `bodySamples >= 10`，
  `PARITY_ONLY` 诊断模式打印取样计数（实测 `thread-title-sync` **20 条样本 / 0 行差异**）。
- 三方字段表同步（`DIFF_ENTRY_FIELDS` / 报告脚本的 `FIELDS` / 签入基线），
  `parity-ledger-fields.test.ts` **当场把没同步的那一步报红了**——那道门真接住了。

## 上一轮（2026-09-16 第二十九轮）做了什么

**agent 会话上那颗分支键关掉了——判词是查后端定的，而且和第一印象相反。**

- **先把一个 0 变成真读数**：`agent-chat` 的 `path` 是 `…/chats/new`，
  一条消息都没有，三颗回合键两边都不画。补一支落在**已有消息的** agent 线程上的
  终态之后，两个语言维各三行：`ariaOnlyVue: button "Branch conversation"` /
  `tabbablesOnlyVue: button` / `tabOrder 第 18 个`。
  **重跑与编辑两颗两边都有**，只有分支是本仓独有。
- 为了换落点给 `ParityState` 加了 `path`（与 `routes`/`dimensions` 同一条理由）。
- **判词换了方向**：`git log -S canBranch` 查到分支是 #3950 加的、
  那次只动了普通会话页、agent 页从没被碰过、也没有注释解释
  ——**看起来像上游漏传**。直到查后端：分支接口不继承 `agent_name`
  （`threads.py:1052-1058` 只写 `metadata=branch_metadata`），
  而 agent 归属就存在线程行的这个元数据里。
  **本仓这颗键点下去会造出一条服务端不认为属于这个 agent 的线程。**
- 所以关掉本仓那颗（显式 `canBranch`，调用点按 `!agentName` 传），
  **而不是给上游补一颗**。改后那两个维度十一个档全空。
- **新账 J**：分支不继承 agent 归属（后端）。没顺手改——后端有自己的 TDD 要求。
  修好之后要一起重判三处（后端、本仓这条、上游那两处漏传）。
- `canBranch` 必须给显式默认值 `true`：**Vue 的布尔 prop 不传时是 `false`**
  （wave 15 那条坑）。负向验证 N9 把它翻成 `false`，三条用例一起红。

## 上一轮（2026-09-16 第二十八轮）做了什么

**账 I 结清：上游那三串交互条件，本仓只兑现了一条。**

- 逐条对完**六项里只有两项是真缺口**：`!isMock` 已由 `interactive` 兑现；
  `!thread.isLoading` 与 `!isNewThread` 本仓**靠结构兑现**（streaming 时
  `latestAssistantGroupId` 与 `branchable` 就空了，键根本不画——
  上游 `message-list.tsx:707/717` 同样是不画，不是画着禁用）；
  `!STATIC` 不补（静态整站模式在对齐范围之外）。
  剩下 `!isUploading` 与 `!branchThread.isPending` 两条，本仓一条都没有。
- **`branchPending` 那一条不只是灰一下按钮**：本仓 `branch()` 可重入，
  连点两下就是两个 POST、**两条新线程**。上游拿的是 react-query mutation 自带的位。
- **那条不对称是判据本身**：上游 `canRegenerate` 里没有 `!branchThread.isPending`
  ——分支在飞时重跑仍然合法。测里专门钉了它，挡「三颗一起禁掉」那种更整齐的写法。
- **顺带订正一段撑着设计决定的注释**：`AssistantTurnActions.vue` 文件头写的
  「上传中、加载中……仍然画出来只是禁用」三处都不对（「上传中」写下来那天就是假的、
  「加载中」两边都是不画、「静态站」在范围外）。第十六轮那条教训的又一个样本。
- 负向验证七条全红。**N1 第一次没生效却全绿**（`old` 串按未格式化的三行写，
  实际是一行），靠「先打印被改对象再看用例」当场发现——第十七轮那条纪律又救了一次。
...
**结清第十八轮留下的 `GoalStatus` 位置差异。一份夹具逼出两处真差异。**

- **这笔账挂了八轮，卡的不是代码是读数。** 账上写「先补夹具」，而 mock 的
  `MockThread.goal` 字段**一直就有**（`threadChannelValues` 里是 `goal: thread?.goal ?? null`）
  ——缺的是**一个同时喂 `goal` 与 `todos` 的场景**。补进 `thread-todos` 之后，
  那两个此前**基线全空**的维度一次报出 18 个投影。
- **改前读数**：目标条 y **React=494.5 / Vue=631.5 Δ137**（上游在待办上面、本仓在下面）、
  `To-dos` 与三条待办各 **Δ-48**、`order` 第 45 个公共节点两边不同。
- **改法一**：`GoalStatus` 从 `ChatComposer.vue` 搬到 `AgentChat.vue` 的 TodoList 同层。
  **外层 `v-if` 要一并放宽成 `(activeGoal || todos.length)`**（上游 `chat-page.tsx:517`
  是 `{(hasGoal || hasTodos) && …}`）——只按待办判的话，「有目标、没待办」的线程
  会连目标条一起不渲染，**那是把位置差异换成缺失**。
- **改法二（第二处，比原账要紧）**：上游 `chat-page.tsx:481` 的 `canEdit` 里有
  **`!hasGoal`**——**有活跃目标时不许「编辑并重跑」**，因为重跑会打断目标的续跑循环。
  本仓 `MessageList.vue` 没有这一条，用户点得到。给它加 `hasGoal` prop 并由
  `AgentChat` 传 `activeGoal`。
- **改后两个维度十一个档全空**，包括 `tabbablesOnlyVue` 与 `tabOrder`
  ——那两行**本来就是同一颗键**的投影。我一度打算把它们归进判过的
  `div[scroll-area-viewport]` 老账，**那会是一次错误归因**。

**两条教训**：①「先补夹具」这类账要先核「夹具到底缺什么」——**「缺夹具」和
「缺一个喂它的场景」是两件事**；② 别急着把剩下的读数归进老账，**改完复量、
看它跟不跟着一起消失**才是判据。

## 上一轮（2026-09-16 第二十五轮）做了什么

**账 H 结清：把上游写死的「不安全链接」两句话搬进词典（两边同改）。**

- **改前读数**（两边都把组件真渲染出来读属性）：本仓 zh-CN 念
  `aria-label="已省略不安全链接"`，上游 **`"Unsafe link omitted"`**——写死在
  `markdown-link.tsx:86-87` 与 `artifact-link.tsx:29-30`，**与语言无关**。
- **改法**：上游 `markdown` 块加 `unsafeLink` / `unsafeLinkTitle`（en/zh 用本仓已有那两句），
  两个组件改读 `t.markdown.*`。**本仓一个字没改**，它本来就是对的。
  与账 F 的分界：账 F 那四条是 **`streamdown` 产物**写死的、上游也翻不掉 → 走 `primitives.*`；
  这两条是**上游自己的源码** → **够得着，两边同改**。
- **放进上游哪一块**：仍是 `markdown`。它一度看着像「就是 `StreamdownTranslations`」，
  实测不是——`ai-elements/streamdown.tsx` 的注释写着 `close` 是**故意不放进去**的，
  所以这一块本来就是「本仓这一面的 markdown 文案，顺便喂给 streamdown」。
- **顺带做成一条零豁免的守卫**：**两边同名的词条，两种语言都念同一句话**。
  先量再立——实测同名条目 en-US **666 条** / zh-CN **709 条**，两种语言各 **0 条**不一致。
  用例加在 `upstream-key-coverage.test.ts`（已在 `CROSS_APP_BY_DESIGN` 登记），
  带形状断言（比较条数 > 600，否则抽取器写坏了会静默全绿）。

**这一轮最值钱的一条教训**：**量法本身先被证伪过一次。** 第一版探针用
`vi.stubGlobal("useNuxtApp", …)` 换语言，两种语言**都读出英文**——看起来像一条干净结论。
实际是 `tests/setup/dom.ts` 把 `$i18n` 放进 `config.global.mocks`，
**模板里的 `$i18n` 走 ctx 而不是 `useNuxtApp()`**，locale 根本没换过。
加一条负向对照（同一个 mock 下 `markdown.copyCode` 必须变「复制代码」）才揪出来。
...
- 顺带量出一条流程账，**但第二十二轮把因果订正了**：规则写着「批不超过 4 轮」，
  而 `e2e-mock` 上一次真跑是第十三轮、隔了 7 轮——**那条红与此无关**。
  逐次查 CI 的 job 步骤后：09-09 那次 `verify` 整个 success（用例当时还不存在，
  09-12 才由 `6e1c06c2` 加），两次推送之间本地攒了 **360 个提交**；09-15 第一次
  推上去时先被 i18n 那条红挡住（后面的步骤全 skipped），修掉才第一次跑到它。
  **本机跑多少遍都抓不到——它在 macOS 上就是绿的。** 变量是推送节奏，不是本机批次。
...
读数：verify exit 0 / **330 文件 2683 单测**；台账 **159 唯一行 / 179 多重集 /
...
**修了第十二轮那四颗按钮的第五个兄弟，并且撞到一条更值钱的：
一次没生效的变异，和「守卫没问题」长得一模一样。**

### 一、一道门钉住了一句仓库已经宣布作废的话

`icon-parity-tool.test.ts` 有一条 `expect(source).toContain("这是顾问工具，不进任何门禁")`，
而 `scripts/icon-parity.mjs` 的文件头**专门标注过那句话从 wave 111 起就是假的**。
它今天能过，只因为脚本把那句假话引在了纠正段里——谁整理掉纠正段，这道门就红，
而红的原因与它想守的事毫无关系。尺子换成真正要守的三样（分支条件 + 那句提示 + `exit(0)`）。

顺带量：全仓把「源码里必须含有某句中文」当判据的只有 12 条 / 3 份文件，
其余 11 条钉的都是**脚本真的会打印的消息**（行为锚点），合理。

### 二、footer 设置键没有键盘焦点环

把「状态条件类」回扫一遍（第十二轮挂的账）。产品面只剩三处，静态对一下 footer
那颗手抄的类串与 `sidebarMenuButtonVariants` 的 `base+lg+default`——
**35 个 token 里它只有 12 个**，活的漏项：
`outline-hidden` / `ring-sidebar-ring` / `focus-visible:ring-2`（**没有焦点环**）、
`transition-[width,height,padding]`、`active:*` ×2、`data-[state=open]:hover:*` ×2。

**这是第十二轮那四颗导航键的第五个兄弟**（同一份 cva、同一个根因）。
那一轮跳过它是因为 wave 74 的豁免，而**那条豁免说的是 `peer/menu-button` 与
收起态尺寸，不是焦点环**——**豁免的范围被读宽了**。

钉法：`upstream-class-echo` 加一条，逐条对着 cva 比，只放过在本仓哑掉的那几条
（每条写清为什么），**双向**。

### 三、负向验证自己也会是空操作（本轮最值钱的一条）
...
对策写进纪律：**变异之后先打印被改对象的前后状态，再看用例红不红。**

### 负向验证（3 条，全部重做过）

| # | 变异 | 该响的 |
| --- | --- | --- |
| N1 | 去掉 footer 键的三个焦点环 token（**定位到元素后改，并打印前后**） | 报出 `focus-visible:ring-2` / `outline-hidden` / `ring-sidebar-ring` |
| N2 | 把 cva 的解析正则写坏 | 形状断言先红（`expected 5 to be greater than 25`） |
| N3 | `INERT` 里塞一条 cva 没有的 | 反向那条报 `bg-fuchsia-500` |

### 顺手改掉的两处措辞

`tests/e2e-parity/support/ledger.ts` 里「可没有任何机器在守」改成过去时
（同一段下面就写着「判据落在 parity-ledger-fields.test.ts」）。
...
**按第十轮写下的清单全仓扫「手抄 primitive 外观」，第一条就打脸：
同一份文件里还有第 4 颗，第十轮没修到它——不是判据不同，是没有锚点落在它上面。**

这是「读数只覆盖取样点」的一个具体样本：台账能证明「这几个点上两边一样」，
而**「手抄 primitive」是分布式缺陷**——它长在哪里，哪里就得恰好有个锚点才看得见。

**扫描面怎么收窄的**：第一版按「变体色对 + 主题色对」扫出 31 处，绝大多数是
主题 token 的正常用法（横幅 / toast / 面板），上游同样直接写——**要豁免表就说明
判据没选对**（坑 180）。收窄成只取 `badgeVariants` / `buttonVariants` 里**真正声明过**
的那三对，**31 → 8**；而且色对清单是**从两个 `variants.ts` 里读出来的**，
primitive 改了变体，扫描面自己跟着变。

8 处逐条对上游：**改 2 处**（`ProcessingToolStep.vue` 的 web_search 链改走 Badge；
`AgentChat.vue` 那两颗「开始对话 / 返回画廊」改走 `Button as-child` 套 `NuxtLink`
——**保留链接语义**，点了就跳走的控件不该是按钮），
**照抄不动 4 处**（上游逐字相同地手写：`HumanInputCard` / `ThreadBackgroundTasks` /
`ThreadSubagentBatches` / `SettingsDialog`），
**留 3 处并写进清单**（两处上游由 streamdown 包内部渲染、没有可对的调用点；
一处是视觉基线夹具页，不是产品面）。

新守卫 `tests/guards/handwritten-variant-colors.test.ts`：
...
**backlog 上那笔 `ui/input-group` 判「不移植」——但不是靠论证，是靠先造一个读数。**

工单自己写着「动它之前先确认台账上它现在是 0 行」。确认了（三个 composer 面
除两条判过的老账外全 0），然后把这句话反过来用：**既然没有读数在推动，
那就先把它接进取样面，再决定要不要动。**

核实「手写」到底手写了什么：上游调用点 `input-box.tsx:2265` 用
`*:data-[slot='input-group']:rounded-2xl` 把 InputGroup 从 `rounded-md` 顶成
`rounded-2xl`，再叠 `bg-background/85 backdrop-blur-sm z-10`；
本仓 `ComposerSurface.vue` 逐条对得上，连焦点环那三条
`has-[[data-slot=input-group-control]:focus-visible]:*` 都照抄。
**它是量着抄的，不是另起炉灶。**

产出：把 `[data-slot="input-group"]`（两边共有的**结构坐标**，第八轮那条教训的
直接应用）挂到**跑满 12 维矩阵**的 `chat` 上。**12 个维度零新增行**，
而且这个 0 是算出来的——探针实测每屏只匹配一份、样本非空、两边逐位相同
（桌面浅色 x=479.5 y=244 576×116 `rgba(255,255,255,204)`；深色
`rgba(31,31,29,204)` fontWeight 300；窄屏 x=12 y=198 351×116）。

判词：**不移植**。高流量组件重写而没有任何读数推动，只会把本仓那块外壳的
实测依据一起扔掉（记忆里「不做搬运式移植」说的正是这种情况）。

**边界写清楚**：几何档**不量 `border-radius` / `box-shadow` / `backdrop-filter`**，
形状类漂移目前没有机器看得见。**翻案判据**：给几何档加 `borderRadius`
（变异论证现成：`rounded-2xl` → `rounded-md`，现有各档一条都不响），
或上游改了 InputGroup 语义。

## 上一轮（2026-09-12 第八轮）做了什么

**给 `project-detail` 补窄屏这一维（第二个「整页」样本），三维全 0 行；
过程里踩出一个新形状的锚点陷阱，写成了一条补充判据。**

这一页原来的两个锚点都是夹具里的**文字**，几何档只量得到两个文本节点，
而窄屏最容易出事的是动作列。补一个动作列锚点之后，第一版
（`role: link, name: /^(New chat|新建对话)$/`）报出一行
`fontWeight React=400 Vue=500`，**只在 en-US 桌面那一维**，连跑两次一样。

`fontWeight` 不该跟语言或断点有关——这个不对称本身就是线索。根因是
**`sidebar.newChat` 与 `projects.newChat` 在英文下是同一个串**（都是 `New chat`），
**中文下不是**（`新对话` vs `新建对话`）：那条跨语言正则在 en-US 桌面上匹配到两份
（侧栏 + 页头），`.first()` 量的是侧栏那颗；zh-CN 与 mobile 只匹配一份。
**三个维度量的不是同一个元素。**

换成按 href 的结构锚点（`a[href*="/workspace/chats/new?project="]`）之后三维全 0。

**判据补一条**：**跨语言正则锚点，要对每一种语言分别问一次「这一屏上有几份」。**
wave 131 那三问里的「只有一份」**本身会随维度变**——两个不同的词典键在一种语言下
撞车，只在那一种语言下现形。**优先用结构坐标（href / data-slot），
可访问名留给没有结构坐标的场合。**
...
**这一轮的货是一条否定结论、一次判词订正，和两个新维度的永久覆盖。**

1. **把第五轮立的判据用到底、然后宣布它见底。** 判据是「『台账 0 行』先问
   『夹具把某一支喂空了吗』」。剥掉注释全量扫 `scenarios.ts`：**21 处空数组 / 空对象**，
   逐条看过——除了第五轮已修的 `channels`，**没有第二处是缺口**。
   写下来就是为了让下一轮别重扫。
2. **`scheduled-tasks` 补 `mobile` 与 `dark` 两维，几何与 aria 各零行。**
   这一页是本仓表单最密的一屏，而**窄屏取样此前一个「整页」样本都没有**
   （`integrations` 那一维是对话框里的叠放）。零行不是「没查出东西」，
   是从现在起有机器守着这一页在 375px 与深色下的排布；深色那一维盯的是
   `load-failed` 那支真的画出来的错误（两边都写 `text-destructive`）。
3. **顺手推翻了自己一句判词。** 新维度报出的 12 行全属于两条判过的账，
   其中 `thread-list-pin#mobile-drawer` 那条的支撑理由
   「本仓做的网络工作严格更少」**只在抽屉被打开过时成立**。
   抽屉没打开时（手机常态）方向相反：上游子树没挂载、一次请求都不发，
   本仓照发三次。判词就地订正成「两种取舍：本仓预取、上游按需」，仍判不跟。

## 上一轮（2026-09-12 第五轮）做了什么

**把第四轮挂的两条存量都结了，其中一条挖出一处真缺陷。**
台账 137 → 154 唯一行、131 → 133 场景-维度——**涨的 17 行全部是新接上的取样面**
（`channels` 的「已连接」终态），逐条判词写在场景注释里。

**一、`icon-parity` 那条「`Unplug` 只有 Vue 用」不是图标问题。**
回源码看：本仓的渠道设置页给每条 connection 画一颗 Disconnect，上游那一页
**没有任何 per-connection 动作**。而台账对 `channels` 报 0 行——因为场景喂的是
`{ connections: [] }`，**「已连接」那一整块两个应用都没渲染过**。
给 `channels` 加一个 `settings-panel-connected` 终态（喂一条真 connection，
锚点用夹具给的账号名），一比就是 **22 行**。

按 wave 73 那条判据问「这处不改，React 自己是不是也是坏的？」——**是**：
后端有 `DELETE /api/channels/connections/{id}`，上游连
`useDisconnectChannelConnection` 都写好了却**零消费者**，而那一页唯一一颗「断开」
是管理员限定、删的是整个部署的 provider 配置。**用户能绑上去，却没法解绑。**
两边同改：把已有的 hook 接上一颗同形的键（outline / sm / `UnplugIcon` /
在飞时转圈 / 可访问名念出账号名），两边词典各补
`channels.disconnect` / `channels.disconnectAccount`。**多账号列表那个概念不跟过来**
——上游这一行本来就只认一条 connection，引进列表属于「引入新概念」。

读数 **22 → 17 行**；`icon-parity` 从「1 处待核」回到 **0 处待核**，
**是从根因清的，不是加豁免**。

**二、`asset-budget` 查清楚了，是存量，按这份文件自己的先例重定。**
抬数字之前逐条问过：不是重复打包（Reka 内部字面量各只出现在 1 个 chunk）、
用户下载没涨（`route-payload` 5 条全绿）、涨的是「带这个名字的 chunk 从 10 个变成 20 个」
（这一格按 chunk 名字归属字节，而把手写标记迁到 primitive 必然让更多产品 chunk 碰到 Reka）、
`maxRaw` 那个 chunk 其实是工作区产品代码（`artifact` 72 次 / `sidecar` 71 次 /
`splitpanes` 1 次）。**红了这么久没人知道的原因**：它只活在 CI 里，而这条分支
三百多个提交从来没推过，CI 一次都没跑过。

**这一轮踩出来的一条**：负向验证的第一版把渲染条件写成 `{false && … ? … : null}`，
**Next 构建直接失败**，报出来是 `Process from config.webServer was not able to start`
——那不是「用例红了」，是用例根本没跑（wave 69「变异必须保持文件可编译」）。
换成把条件反过来才拿到真红，失败点精确停在 `expect(disconnect).toBeVisible()`。

## 上一轮（2026-09-12 第四轮）做了什么

**两件事，各自带负向验证。台账 154 → 137 唯一行（170 → 153 多重集），场景-维度 129 → 131。**
逐条判词在 `vue-parity-open-accounts.md` 顶部那两条 2026-09-12 条目。

**一、窄屏那两簇结清（`integrations` 86 → 68）。** 上一轮留的工单，四条假设里
**第四条被推翻了**：「给 ScrollArea 加 `[&>div]:block`，读数 86 → 86」是**假阴性**
——Radix 把 `min-width:100%; display:table` 写成**内联样式**
（`@radix-ui/react-scroll-area@1.2.10` 的 `dist/index.mjs:130`），Tailwind 生成的
普通 CSS 规则顶不掉它，那次变异从来没生效。**否定结论同样要先证明变异生效。**

根因逐层量出来了：375px 屏上对话框 343 宽，面板 `p-6` + 这一页独有的 Card `px-6` +
状态盒 `p-3` 三层内边距吃掉 208px，内容列只剩 **167px**；而里面
`Re-register in browser` 那颗按钮的 min-content 是 **191.1px**、scope 示例那句 `<p>`
是 **192.2px**。上游让 shrink-to-fit 把整个面板撑出栅格格子 25.2px，本仓把内容裁掉
——**两边都坏，坏法不同**，所以几何档只报得出「宽度不一样」。
两边同改成 `p-4 sm:p-6` + `px-4 sm:px-6`（`sm` 以上逐字不变，桌面维度与视觉基线零变化），
内容列 167 → 199，全链零溢出。

**二、diff 三档进取样面 + `workspace-changes` 补一维 dark。** 上一轮给 diff 三档补上
`dark:text-*-300` 时台账零反应，原因是**那一块只有一个 heading 锚点**。挂上三个锚点
（文本取自夹具 diff：`+Ready` / `-Draft` / hunk 头）之后：颜色两边逐值相同、深色下正确翻档
（从此有机器守着），**同时当场报出 9 行 `y Δ4`**——面板正文上游是 `px-5 py-4`、
本仓写成 `p-5`，这 4px 把面板里每一行都往下推。本仓改成 `px-5 py-4`，9 行归零。

**新守卫**：`tests/unit/parity/workspace-diff-anchors.test.ts` —— 每个文本锚点必须
**恰好**命中夹具 diff 里的一行，且 addition / deletion / hunk 三档各被盖住一次
（归类走产品代码自己的 `getWorkspaceChangeLineClass`）。锚点指不到东西时两边都记 `null`、
`diffGeometry` 跳过，台账 0 行而没有任何用例会红——这条守卫挡的就是那个。

**两个应用各加一条 375px 零溢出用例**（`integrations.spec.ts`）：
`panelOverflow` 抓上游那种「撑出格子」，`cardOverflow` 抓本仓那种「裁掉」，
两个读数缺一不可。**负向验证 2×2 全红**：

| 变异 | React | Vue |
| --- | --- | --- |
| 只还原面板内边距 | `panelOverflow 9 / cardOverflow 6` | `cardOverflow 15` |
| 只还原 Card 内边距 | `panelOverflow 9` | `cardOverflow 7` |

守卫自己的变异也做了两条：改掉夹具那一行 → 2 条红；拿掉 hunk 锚点 → 1 条红。

**一处如实记下的「量不出效果」**：`WorkspaceChangesBadge.vue` 的 `SheetContent`
从 `sm:max-w-none` 改成上游的 `sm:max-w-[900px]`，**今天两者渲染一模一样、读数零变化**。
留下它的理由不是量出来的，是两个 token 语义不同。

**三、九条门禁逐条真跑，撞出两条一直红着的存量**（上一轮的读数块两条都写着 exit 0）：

- `icon-parity` 的 `exit 1` 来自一条**过期豁免**（`ChevronLeft` 现在两边都在用，
  那条 `VERIFIED` 是死配置）——按 stale 的提示回去看过一遍、删掉，回到 exit 0。
  **剩下的 1 处待核是真的**：`Unplug` 只有 Vue 用，背后是**一处 Vue 独有的产品面**
  （每条 connection 一颗 Disconnect，上游那一页没有任何 per-connection 动作）。
  它**一直都在**，只是 2026-09-11 之前被「两边碰巧都用这颗图标」盖住了。
- `asset-budget` 四条预算全超，**证过因果**：把本轮动过的三份 `app/` 文件换回 HEAD
  重新构建再量，读数只差 16 字节。是存量，最可能来自 2026-09-10 那次上游合并。

两条都写进了 `vue-parity-open-accounts.md` 顶部，各带翻案判据，排在下一轮第 0 位。

## 历史快照（截至 wave 202，2026-09-09；数字已过期）
...
- 分支 `main-wc`。`b700cf17` = wave 39（chore `b09adb80`），
  ...
  `b73643e3` = wave 198（**拖拽用例在过渡途中量分隔条**：命中区 16px、过渡里滑 332px；
  共享 `settledBox` 按真实时间间隔判稳，并当场断言「抓住了没有」）。
  `dbd1e2a5` = wave 199（**合成 wheel 不会真的滚动**：列表还在尾部，`onScroll` 的
  `atTail` 分支把 `followingTail` 置回 true，虚拟窗口钉死在尾部；改用真滚轮，
  并把那个致命事件永久留在用例里）。
  `0391ecc5` = wave 200（**挂账清空**：`/auth/callback` 的挡路理由本身是错的，接进取样面；
...
① 需要交互才看得见的；② ~~藏在请求 body 里的~~ —— **第三十轮补上了**
（`ParityCapture.requestBodies` + `DiffEntry.requestBodies`：只比**两边都发过**的
那些键，体按 key 排序并把客户端生成的 UUID 抹成 `«generated»`，
与 `normalizeRequest` 同一条规则同一份名单；**没有第三条归一化规则**，
时间戳一律不抹——真出现了会作为一条 body 差异报出来，拿着读数再加）。
配了一条形状断言 `bodySamples >= 10` 挡住「采集坏掉→永远空数组→静默 0 行」，
`PARITY_ONLY` 诊断模式也会把取样计数打出来；
③ portal 出去还会遮蔽页面的浮层；
...
`DEERFLOW_WIRE_EVENTS` 的头写着「当前 Gateway 会发出的 wire 事件名全集」，
而没有任何机器在对。**wave 107 判的是「有意不钉」**，理由是后端没枚举、名字散在
`bridge.publish(run_id, <mode>)` 上、照那个扫会漏——**那个判断当时是对的**，
并留了翻案判据：「后端哪天给 stream mode 也定了枚举，这条就可以照着补」。

### 量到判据已经成立

`runtime/stream_modes.py` 里已经有 `type RunStreamMode = Literal[…]`（7 个）。
剩下 4 个不是 stream mode，各自有出处：`metadata`/`error` 在 `runs/worker.py`，
`end`/`gap` 在 `app/gateway/services.py`。对齐关系是精确的：

    8 个业务事件 == {"metadata"} ∪ RunStreamMode（messages-tuple 折算成 messages）
    3 个控制事件 == {"error","end","gap"}，逐个钉到发它的那份文件

**那处改名本身也断言**，否则后端改掉转换、这张表却不会响。
4 个控制/元数据事件**不是「另一半不检查」**，是换一种方式检查——正是挂账清单
那条判据（「一张表把全集切成两半，而另一半的处理方式是『不检查』吗？」）。

### 负向验证

多一个 `telemetry` → 红；去掉 `tasks` → 红；后端去掉那处改名 → 另一条红。
**三种坏法各红在对的那一条上。**

### 这一轮真正的收获

**翻案判据留着是有用的，它让一条「现在做不了」不会变成「永远不做」。**
wave 107 没有写死「不做」，而是写了「什么时候可以做」；四轮之后条件自己成立了。
挂账清单里「还没筛的同形目标」那条也已过期——另一张表 wave 107 就钉住了。

## 上一轮（wave 200）做了什么：**挂账清空——两条都是「挡路的理由本身是错的」或「同一族的界面没停稳」**

**机器可读的三张 pending 表现在全是 0。**

### 一、`/auth/callback`（路由棘轮最后一条）

挂着的理由写的是「要一次真实的 OIDC callback session，回放 Gateway 不提供」——
**是猜的、而且是错的**。两边的实现都只做一件事：问一次 `GET /api/v1/auth/me` 再分支
（React 三态、Vue 四态）。用 `delayMs`（wave 134 就是为「加载中」加的）把那个端点挂住，
它就从一闪而过变成稳定终态。**wave 189 踩过同一个坑。**

读数：**en-US 十一档全空；zh-CN 只有 2 行**，属已判过的「上游写死英文、本仓翻译了」。

**锚点换过一次**：第一版锚 `.animate-spin`，zh-CN 报 `width React=41.3 Vue=44.7`
——`h-8 w-8` 是 32px，41.3/44.7 是**旋转中的外接盒**。而 `animate-spin` 在
`looping-animations.json` 里是 `decision: "always"`（有意常开），**不能顺手门控**，
签进去就是签一条必红的行。改锚外层全屏容器；**十一档里只有 `geometry` 比锚点**，
其余十档比整屏，所以换锚不丢信号。

### 二、`integrations.spec.ts:461` 偶发点击超时

**对话框还在缩放进场时点它里面的东西。** `data-[state=open]:zoom-in-95` + `duration-200`，
实测位移：
...
**21.2px**，比很多按钮半宽还大。与 wave 198（14px）同族。
改法是收成唯一入口 `openSettingsDialog`（导航 → 等出现 → `settledBox` 等停稳），
12 处深链全走它，并配零豁免守卫。**不是在出事那几处各补一句「记得等一下」**——
那是要靠人记住的规矩，忘一次就回到偶发红。

### 三、两条自己造的错，都记下来

- **我引入过一次回归**：把 `i18n-theme` 那处的 dialog locator 锁成
  `enUS.settings.title`，而那条用例随后会切语言、可访问名变成中文，当场红。
  `name` 因此做成可选。**会切语言的用例不能把 locator 锁在某种语言的名字上。**
- **第一版探针不成立**：拿 `?settings=general` + `calendar` 按钮，而那个按钮要
  Lark 状态 mock 才有——**探针自身夹具不对，不采信**，改成直接量位移。

### 四、顺带订正了一批过期数字

散文里写着「台账 202 行 / 90 个取样点」，**实测是 103 行 / 95 个取样点**
（`scripts/parity-ledger-report.mjs`）。挂账清单第一节那个标题「真正还开着的（4 条）」
也是历史文本，下面的行早就逐条判过。**别引用散文里的数字，跑脚本。**

## 上一轮（wave 199）做了什么：**合成 wheel 不会真的滚动——列表还停在尾部，虚拟窗口被钉死在那里**

wave 196 挂的两条低频红收掉第二条（`thread-history.spec.ts:105`）。**两条都清了。**

### 根因

`dispatchEvent("wheel", { deltaY: -1000 })` 是合成事件，**不会真的滚动**。
而 `MessageList.onScroll` 的第一条分支是「在尾部就恢复跟随尾部」：

    if (atTail) { followingTail.value = true; windowStart.value = maxStart; return; }

元素还停在尾部，于是**任何一个**随后落下的 scroll 事件（例如上一次
`scrollToTail("smooth")` 的平滑滚动还在发）都会把 `followingTail` 置回 true；
再往下用例把 `scrollTop` 设成 0 发 scroll，走的是
`if (followingTail.value) { windowStart = maxStart; return; }`
——虚拟窗口钉死在尾部，首条永远不渲染，而 `scrollTop` 明明是 0。
...
| --- | --- | --- |
| 原样 | 0 | 1 |
| wheel 后补一个「尾部」scroll | 0 | **0** ← 复刻失败 |
| 真滚轮 + 同样补那个 scroll | 0 | 1 ← 修法成立 |

**两条先被排除的假设**（都实测，不是推理）：不是 `scrollTop` 被弹回去
（设成 0 后采样 2 秒，1x/6x 节流都稳在 0）；不是选错滚动元素
（量到的就是那个 `overflow-y-auto`，scrollHeight 161072 / clientHeight 548）。

### 改法

`scrollLogToTop()`：先用**真滚轮**把列表带离尾部（poll 到 `scrollTop` 真的降下来），
再归零。**并把那个曾经让它翻车的 scroll 事件永久留在用例里**——真滚轮之后它落不进
`atTail`，不影响用例；一旦有人换回合成 wheel，用例**确定性地**红。
负向验证：换回去 `--repeat-each=3` **3/3 红**。

### 复现循环的两条读数（值得记）

- **`--workers=12` 是过度放大**：整套 25 条红、散在 5 个无关 spec 上，
  目标用例反而不在其中。**那是另一种状态，不能当同一个 bug。**
- 默认并行度整套跑 **10 轮只红 1 条**，还是又一条不同的用例
  （`integrations.spec.ts:461` 点击超时，现场由 wave 197 的归档器留下了）。
  所以这一轮**不是靠复现**，是靠把机制在代码层跑通。

**顺带说明这一段的形状**：196（历史被吞）、198（在过渡途中量坐标）、199（合成事件
不产生它看起来产生的状态）是同一类——**在还没稳定的界面上量一次、然后照那个读数行动**。

## 上一轮（wave 198）做了什么：**拖拽用例在过渡途中量分隔条——按空了，报出来却是「面板没关」**

wave 196 挂的两条低频红，收掉第一条（`artifact-panel-resize.spec.ts:106`）。

### 量到的

artifact 面板打开时 splitpanes 给窗格的是 `width .2s ease-out`（它自带的样式），
分隔条 200ms 内从 **1119 滑到 787**（332px），而它的命中区只有 `::after` 的 **16px**。
...
**按空之后整个拖拽静默地什么都不做**，最后由十秒后的 `toBeHidden()` 报出来。
wave 196 那次红的 trace 里量到的中心是 **801**，正落在 889 与 787 之间。

### 被证伪的那句话

两个 spec 都写着「`hover()` 的可操作性等待包含『元素已停止移动』」。
**它判的是相邻两个动画帧的盒子相同**——整套并行跑（本机默认 6 个 worker）时
渲染帧会被饿着，两次 rAF 之间可能没有重新排版，于是提前判稳。

**帧饥饿本身没能单独复现**：CPU 节流 4x/6x/20x 复现不出来，把过渡换成尾巴极慢的
缓动也复现不出来（漂移 0–1px）。**所以不声称复现了它**——能确定性证明的只有上表。

### 改法与两处判据

`tests/support/settled-box.ts`：按**真实时间间隔**判稳。

- **「连续 3 次、间隔 100ms 都不动」，不是「两次读数相同」。** 两次不够——
  面板刚可见时窗格宽度还是 0、过渡才要开始，两次快读拿到同一个**起始**位置，
  当场判稳、按在 1091 上照样抓空（第一版就是这么写的）。三次跨度 300ms > 200ms 过渡。
- `dragPanel` 在**第一次移动之后**断言 `splitpanes--dragging` 挂上了没有。
  不能放在 `mouse.down()` 后面：splitpanes 在 mousedown 上只设内部的 `mouseDown`，
  `dragging` 要等第一次 mousemove。

### 一条没做成的负向验证

把 `SETTLE_SAMPLES` 降到 1 跑 `--repeat-each=3`，**15/15 全绿**——
那条路径的失败本身也是低频的。**这条实验不构成证据，没拿它当证据。**
真正的负向验证在单测层（改回 1，5 条里当场红 2 条）。
...
### 一、安装页那 5 处，全在 `frontend-vue/` 内

wave 194 照出来的五条，逐条修：

| 差异 | 修法 |
| --- | --- |
| 缺副标题 | 补 `setup.createAdminSubtitle`（两份词典 + types） |
| 「保持登录」缺说明 | 照 `login.vue` 那份**与上游 RememberSessionOption 逐字同构**的写法 |
| 确认密码缺占位符 | 新增 `setup.confirmPasswordPlaceholder`（此前占位符与标签共用同一条词条） |
| 标签大小写 | `confirmPassword` 改成 `Confirm Password`（上游 label 是大写 P，placeholder 才是小写） |
| 输入框 42/16px vs 36/14px | **全部改走 `Input` primitive**，字段分组照上游 |

> **最后一条是一笔旧账**：`login.vue` 的文件头里早就写着同一段读数——
> 「裸 `<input class="… px-3 py-2">` 实测 h 42 / 字号 16px，而上游的 `<Input>` 是
> `h-9` + `md:text-sm` → h 36 / 14px」。**wave 68 系统修过登录页，漏了安装页**，
> 而安装页直到 wave 194 才第一次进取样面——**这个缺口挂了 126 轮**。

**读数**：`setup-screen/en-US` **10 行 → 1 行**（只剩已判过的 `auth/me`）。
zh-CN 剩 23 行，全部是「上游写死英文」那一类；其中**「保持登录」那对行已彻底消失**
——上游那句是翻译过的，本仓补上之后两边一字不差。

zh-CN 多出 3 行（中文副标题、中文占位符、确认密码框多了占位符），
**是预期增长**：这几处从「Vue 根本没有」变成了「两边都有、只差语言」，
对应的英文行本来就已经记在 `ariaOnlyReact` 里。走 `PARITY_ACCEPT_GROW=1`。

### 二、一条 e2e 用例被「改对了文案」打红——脆的是选择器

`getByLabel("Email")` 现在同时命中邮箱框与「保持登录」复选框，
因为那句说明里含 "email"（**子串匹配**）。改成 `exact: true`。

### 三、`sidecar-chat` 那条偶发红：**根因没找到，但找到了「为什么查不下去」**

现象：`sidecar-message-list` 里的消息在**关闭再打开侧边对话之后** `element(s) not found`。

按 wave 108~113 立的规矩做实验：

| 实验 | 结果 |
| --- | --- |
| 单跑那条用例 ×8 | **8/8 过**，每次 3.7~6.2s |
| 整份 spec ×3（24 条） | **24/24 过** |
| 整套 e2e ×3（269 条） | **269×3 全过** |

**时间账不支持「机器慢」**：失败那次 16.7s ≈ 6.7s 正常流程 + 10s 干等，
前面每一步都是正常速度，只有那一个元素始终不出现。**更像真竞态。**

**查不下去的地方是具体的**——现场只有一张截图，没有网络与 DOM 时间线。读配置才发现：

```
retries: 0
trace: "on-first-retry"     ← 只在**重试**时录；没有重试，就永远不录
video: "retain-on-failure"  ← 就在它旁边，两条策略自相矛盾
```

**这条设置从来没有触发过，而且是静默的**：跑一万次也没人会发现 trace 目录一直是空的，
直到某天真出了一次偶发红。改成 `retain-on-failure`（只在失败的用例上保留，绿的丢弃，
正常跑一遍不多花存储），并**实测验证**：故意让一条用例失败，
`test-results/e2e/…/trace.zip` **确实写出来了**，改之前不会。

配 `tooling-contracts` 的守卫：**`retries` 为 0 时不许用只在重试时才录的模式**，
两次变异各红一条（改回 `on-first-retry`、把那一项改名）。

> ⚠ **必须说清楚**：`sidecar-chat` 那条**本身的根因仍然未知**——此后连跑 14 次全绿，
> 复现不出来。我修的是「下次发生时能不能查」。wave 194 那条
> `integrations#permission-request` 的几何抖动同理，**下次出现时会有 trace 可看**。

- 门禁：`make verify` **275 files / 2248 tests**、`e2e-parity-auth` accept 通过、
  `e2e-mock` **269 + 22 + 15 + 2 + 6 全过**。只动了 `frontend-vue/`，**不需要 marker chore**。
...
wave 190 备好了共享判词，这一轮按计划建 `e2e-parity-auth`：新 config、新 testDir、
新 spec（复用同一把尺子）、新 baseline、两个 make 目标，README×2 与三条守卫都接上了。
**跑起来了，也生成了基线——然后读那份基线，发现它记的不是差异，是我的 config 搭错了。**

### 读数

| 屏 | React | Vue |
| --- | --- | --- |
| `/login` | **登录表单**（email / password / Sign In / 保持登录） | **工作区聊天页**（侧栏、欢迎语、输入框） |
| `/setup` | **也是登录表单**（它自己跳到了 `/login`） | 工作区聊天页 |

**两边根本不在可对照的状态。** 第一版没接 Gateway（照 `e2e-auth` 抄的，而那一套是在 spec 里
用 `page.route()` 自己 mock 的），我以为是这个原因；接上一个**开着鉴权**的回放 Gateway 之后
**读数一模一样**——所以不是「缺后端」。

### 量到的那条线索

请求档把原因指出来了：**两个应用判会话用的端点不同**。

- React：`GET /api/v1/auth/providers`、`GET /api/v1/auth/setup-status`
- Vue：`GET /api/v1/auth/me` ×4

在分清「这是产品差异」还是「回放 Gateway 的种子会话（`DEERFLOW_ENABLE_TEST_SEED=1`）
让两边看到不同状态」之前，**这份读数不能签入**——签进去就是把噪声当成发现，
而且它会以「已判过的台账行」的身份长期误导后面每一轮。

### 所以整体撤回

`playwright.parity-auth.config.ts`、`tests/e2e-parity-auth/`、`baseline/parity-auth-diff.json`、
两个 make 目标、README×2 与三条守卫的改动**全部还原**。工作区回到 wave 190 的状态。

**保留的是量到的东西**：路由棘轮里 `/login` `/setup` 两条的挡路理由已从
「需要开鉴权的 preview」换成实测记录——包括「开了鉴权也还不可对照」和上面那条端点线索。

**下一轮先单独回答一个问题**：把两个应用放进同一个「确实没有会话」的状态，
它们各自显示什么？答得出来，这套对照才建得起来。

- 门禁：无代码改动（撤回后工作区只有 baseline 的理由文字变化）。

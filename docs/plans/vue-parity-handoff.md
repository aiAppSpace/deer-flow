...
> - **台账已经清零（第三十六轮），但清零不等于对齐完成。** 台账只覆盖 147 个
>   场景-维度，取样面之外它一个字都没说；覆盖率棘轮是 covered 37 / pending 1 /
>   exempt 3。**边界说明与下一轮三条方向写在 `NEXT-WINDOW.md`，那份是当前版本。**
>   下一件最大的事：**把 `e2e-parity` 接进 CI**——`.github/workflows/` 里零命中，
>   这把尺子从来没被这台笔记本以外的机器跑过。

...
> - 对照台账 **154 唯一行 / 129 个场景-维度**（数 `baseline/parity-diff.json` 的
>   `entries`；量法写在冷启动文档里）；
> - **「台账还剩多少行」已经不能当坐标系**：154 行里 87 行是三条判过的账、
>   20 行是窄屏那个开着的工单，而且 2026-09-11 那一轮**主动把台账做大了**
>   （新开 `dark` 与 `mobile` 两个取样维度）。

...
## 门禁这一轮拦了我四次（全是我自己排队排错）

| 拦我的 | 我做错了什么 |
| --- | --- |
| e2e 独占锁 ×2 | 上一轮还没收尾就发起下一轮 |
| `make build` 的 e2e 闸门 | parity 还在跑就跑 `make verify` |
| Nuxt 自己的构建锁 | 两轮 `make verify` 重叠 |

**代价不是零**：并发那两轮里有一轮 `make verify` 报出 317 文件 / 2615 条，
而静默的树上是 319 / 2620。数字对不上没放过——重跑确认是并发采样，不是回归。
**跑长门禁前先看锁**，别靠事后对数字。

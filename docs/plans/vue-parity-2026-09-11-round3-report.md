...
分支 `main-wc`，11 个提交（`6ab3cfb3` … `12ae4221`），全部已提交、**未推送**。

## 一句话

对照台账 **330 → 106 行**，而且**剩下的每一行都判过了**（87 行是三条早就判过的账，
真正还开着的 19 行逐条写了判词与翻案判据）；随后**把取样面往深色主题与窄屏各开了一扇窗**，
台账回到 156 行——dark 那一维带进来的 17 行零条新差异（判过的账的投影），
mobile 那一维带进来的 37 行里**有 20 行是真的**：375px 下两个应用的设置对话框布局不同，
其中**上游有一颗真的够不着的控件**（技能开关横向跑出视口）。
台账涨回去不是退步，是**以前看不见的地方现在看得见了**。
...
| 全量多重集 | 330 | **172**（开 dark/mobile 两维之前是 118） |
| 全量唯一行 | 263 | **156**（开两维之前是 106） |
| 取样场景-维度 | 115 | **129** |
...
| `integrations` 的 dark 维度 | 深色主题下的分叉：固定色（`red-600`/`bg-red-50`）在浅色下与 token 一模一样，只有深色才现形，而此前只有 `chat` 一个场景跑过 dark |
| `integrations` 的 mobile 维度 | 窄屏下的布局分叉：**加上去当场量出上游一颗够不着的控件**（技能开关横向跑出 375px 视口） |

...
**上一轮（第五十五轮）是完整收工的**：代码已推送、全套门禁本机全绿、
工作树干净。**但它开的那条链只修到一半**——所以这一轮是
**从「第一步」1a 把它修完**，不是造新面。
...
1. **复核第五十五轮那次推送的 CI**（推完时还在 `in_progress`）。
...
**第五十五轮开了「点得动吗 × 200% 文本」这个面，但那条链只修到一半。**
这一轮先把它修完——**这是唯一一条「已经量好、判好、只差改」的活**，别去造新面。

### 1a. 把 200% 下「popper 比视口宽」那条链修完（**读数都在，直接动手**）

第五十五轮把最外层修了（移动端抽屉 `18rem` → `min(18rem,calc(100vw-3rem))`，
两应用同改，实测 576→279、⋯ 触发器从 x=511 回到 x=214）。**里面还剩两截**：

```
父菜单   改后 rect=[-46,289 256x523]   **两应用同值** → 两边一样坏，不是对照缺陷
子菜单   本仓 [31,490 344x146]  上游 [201,490 344x146]  → **两边位置仍不同**
场景：thread-list-pin#mobile-drawer @200%，视口 375
```

⚠ **这两条路已经走过、被读数否掉了，别重走**（详情见 open-accounts 第五十五轮）：

| 试过的 | 结果 |
| --- | --- |
| 上游 `DropdownMenuSubContent` 传 `sticky="always"` | **读数一格没动**。Radix 的 shift 是 `crossAxis: false`，右侧子菜单根本不在那条轴上 shift |
| 两边补 `max-w-(--*-content-available-width)` | **上游生效**（344→256、条目换行），**本仓 reka 侧两个变量名都不生效**（namespaced 与 `--reka-popper-available-width` 都试过）→ 只有一边变 = 造出新的不一致，**已整组还原** |

**下一条建议**：本仓 `DropdownMenuSubContent` **故意不 portal**（见该文件头的判词），
先量清楚 reka 在不 portal 的子菜单上到底设不设那几个 `--reka-*-available-*` 变量
（`getComputedStyle(el).getPropertyValue(...)` 直接读），**再决定改法**。
⚠ 别为了让变量生效就去 portal 它——那条「不 portal」是 wave 95 拿可访问性树读数判的。

### 1b. 修完之后**才**把这个面做成常驻门禁

⚠ **第五十五轮刻意没有当轮常驻**，理由要读懂再动：那时候这一面还剩
**只有本仓 3 · 只有上游 13 · 原因不同 2** 条稳定差异，
现在常驻就要配一张十几行的登记表——而**第五十四轮刚证明登记表会长假账**
（12 行里 3 行根本不是产品的事）。

**顺序是：先把 1a 修完、把行数压到 0，再常驻。**
探针全文（已验过两侧自检）在 open-accounts 第五十五轮末尾，直接拿去用。

⚠ **探针的两条自检一条都不能删**：
阳性对照（盖整屏遮罩必须报满）与 `inertSkips`/`modalSkips` 计数。
这一轮**两次「整场报 0」全是它们抓到的**，没有它们这把尺子会绿着交付。
...
### 1d. 还没量过的两条轴（**建议，不是判过的账**）

| 候选 | 为什么值得量 | ⚠ 先看 |
| --- | --- | --- |
| **`forced-colors`（Windows 高对比）/ `prefers-contrast`** | 两边 primitive 不同构，强制配色下谁把 `border`/`outline` 画丢量得出来 | 对照上下文已钉了 `reducedMotion: "reduce"`，别和它搞混 |
| **同一条流两次进入之间的幂等** | 第四十八轮那条飘是「第二次不一样」，但那是跨应用比；**同应用两跑自比**还没做过 | 自比要用**新开 page**（第五十轮：遍历本身可能改变被测对象） |
...
- **每一类找到过的缺陷都有一条会红的门禁** ⚠ **第五十五轮起不再是 ✅**：
  「**固定 rem 尺寸的容器在 200% 文本下宽过视口**」这一类**没有门禁**
  （第五十四轮的工具条/状态格、第五十五轮的移动端抽屉都属于它）。
  常驻的前提是先把第一步 1a 那两截修完、把行数压到 0——理由见 1b
...
```
<第五十五轮>  **两个应用都改了移动端抽屉宽度**，**推完必须自己复核这条 run**
f3745b59  第五十四轮   verify + parity **双绿**（attempt 1，第五十五轮收工前查到 completed/success）
...
e2e-parity  0   **210 passed / 48.8m**      ← 第五十五轮实测（台账那条 18.4m）
...
e2e-parity 用例数                             210   （第五十四轮没动；53 轮 +2 overlay-survives-resize；52 轮 +2 content-reachable）

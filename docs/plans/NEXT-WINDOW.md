...
第四十一轮给 `settings-narrow-screen.spec.ts` 加了反空转断言（该有列表的分区
必须真的画出行），**十个分区已经全部量到了内容，别重做那一笔**。

要做的是问同一个问题：**别的门禁有没有在空壳上通过？**
已知同形的两次：`background-tasks#disabled` 的主角断言在空 locator 上绿了很久
（第三十八轮），`settings-narrow-screen` 对 channels 绿了十轮而量的是空面板
（第三十九轮）。**这是本仓出现次数最多的一类失效。**

做法：挑一条现有门禁，问「把它要证明的东西拿掉，它会不会红」——
这正是变异验证，只是对象换成了**夹具/前置条件**，而不是产品代码。

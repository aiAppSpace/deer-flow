...
**E 组看截图就看出来了，而且很可能和 A 组同根**：表格面板顶上那条警告
**React 折成两行、Vue 排成一行**，下面整张表因此错开一个行高（~18px）。
一条文本折不折行取决于容器宽几像素，而 A 组量到的正是同一量级的 ~4px。
zh-CN 只有 Δ-2.1 也吻合（中文那句更短，两边都不折）。
**但方向相反**：这里 React 更窄，integrations 那边 React 更宽（203.1 vs 199）
——所以**不是整体缩放，是某一层容器的盒模型在两处各自不同**。

⚠ **看截图前先钉归属**：`diff.spec.ts`（`captureScenario` 的两次调用，vue 在前） **先采 Vue、再采 React**，
所以 `test-failed-<2i+1>` 是 Vue、`<2i+2>` 是 React。别按奇偶猜——我猜反过一次。

**飘的那 9 行有一条尚未验证的假设**：它们全是 `background`（hover 态）与 `hit`
（命中谁），**两样都由鼠标位置决定**；而两个应用的布局本来就差 4px，
同一组点击坐标可能一边落在按钮上、一边落在按钮外。
验法是取样前把指针移到中性位置，然后**连着量两次以上**。

> ⚠ **本轮在同一个形状上栽了三次，务必读**：形状是**「说得通的东西被当成读数」**。
> ① 用算术（`230/255 = 90.2%` 恰好是 Tailwind `/90` 档）推翻一次实测；
> ② 用源码结构（`CardTitle` 是块级 div）排除字体——而 React 自己的注释里记着
> 那一列卡在 min-content 上，**宽度确实是文本派生的**（结论没变，理由是错的）；
> ③ 用**一次**运行的「16 → 8」判定根因——下一次同一棵树又回到 16。
>

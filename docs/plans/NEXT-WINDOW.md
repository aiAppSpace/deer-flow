...
### 1. 把窄屏扫描做成常驻门禁 —— **设计已判，照着写就行**

第三十八轮已经把它当一次性探针跑过，**43 个状态里量出 1 条真缺陷**
（`subtask-card` 漏抄 `min-w-0`，状态图标被推出视口），而其余所有门禁都没看见。
**值得常驻。**

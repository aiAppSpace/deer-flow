...
那份是当前版本，这里不重复。**五组里判掉了四组（B / C / D / F，共 8 行）**：根因是取样器用固定 700ms 静置、
不等动画结束。修法 `waitForFiniteAnimations` + `animation-settle.spec.ts`，
**判据是同一台 runner 上的复量：16 行 → 8 行**，消失的正好是那四组
（run 35143922501 → 35181791622）。

**剩下 8 行是 A 组与 E 组，未判**：

- A（5 行）：`integrations` 三个 mobile 档的 card-title / textbox 宽度 Δ≈4.1–4.2px；
- E（3 行）：`artifact-table-preview` 的 y 偏移 Δ-18 / -2.1。

字体栈两边确实不同（React 在 `@theme` 定义 `--font-sans`，Vue 用 `body` 三项栈
覆盖掉、丢了四个 emoji 兜底），**但它解释不了 A 组的数字**：76.3/72.1 与
203.1/199 的比值是 1.058 与 1.021，**不一致**，所以不是缩放；两处的差都是
≈4.15，是常量偏移。**这是嫌疑，别当结论写进代码。**

> ⚠ **本轮在这里栽过一次，务必读**：B+D 那组的判词被我用「`230/255 = 90.2%`
> 恰好是 Tailwind 的 `/90` 档位」+「上下文带 `reducedMotion: reduce`」推翻过一次，
> 还写进了代码注释和三份文档——**第二次 CI 运行把它按回去了**。
> 两条推理分别错在「档位接近只说明过渡的终点是那个 token，不说明读到的是终点」
> 和「`reducedMotion` 只改 media feature，不会停掉没被媒体查询包起来的
> `transition-colors`」。**推翻一次实测只能靠另一次实测**，算术再漂亮也只是线索。

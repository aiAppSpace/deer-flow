...
那份是当前版本，这里不重复。**第三十七轮收工时的 Linux 台账（run8 之后又修掉一笔，下一轮先复量）**：

| 账 | 行 | 状态 |
| --- | --- | --- |
| **A 组**（`integrations` mobile 宽度 Δ4.1/4.2） | **0** | ✅ **已结清**——根因是 `ScrollArea` 的四处基类差（run8 实测一条不剩） |
| **账 J**（拖拽手柄缺 `data-slot`） | **0** | ✅ 已修，本机复量 2 维度各 2 行 → 0 |
| E 组（`artifact-table-preview` 的 y 偏移） | 3 | 未判。那条警告 React 折行、Vue 不折；那一屏是原生 `overflow-auto`，**不是 ScrollArea**，所以未必与 A 同根 |
| **账 K**（菜单项焦点在指针移出后不收） | 6 | 未修，**已判机制**，见挂账文档三之四 |
| `hit React=self Vue=div` | 2 | 未判，疑与账 K 同源 |

**起点 16 行 → 中途因尺子变准涨到 41 行 → 实质剩 11 行，而且每一行都认得出是谁。**

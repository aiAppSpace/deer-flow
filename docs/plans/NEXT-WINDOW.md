...
**上一轮（第五十七轮）是完整收工的**：已推送、门禁全绿、工作树干净。
那一轮是**纯负结果**（`forced-colors` 0 条），所以**没有产品改动**。

⚠ **2026-09-20 用户拍板了收尾方案：走 B——「走完已点名的名单再收」。**
名单上**只剩最后一个面**（下面 1a）。**走完就进收尾判定，不要再无限造新面。**
...
**方案 B 的名单只剩最后一个面了。** 走完它就进收尾判定。

### 1a. 开「同一条流两次进入之间的幂等」这个面（**名单上最后一条**）

第四十八轮那条飘是「第二次不一样」，但那是**跨应用**比；
**同应用两跑自比**从来没做过。问的是：

> 同一个终态、同一个应用，**新开一个 page 再跑一遍**，
> aria 树 / 几何 / 请求序列与第一遍一样吗？

⚠ **自比必须用新开 page**（第五十轮判词：遍历本身可能改变被测对象）。
⚠ **骨架照抄 `viewport-fit.spec.ts`**——那是这一族里自检最全的模板
（反空转 + 放大落地断言 + 登记表带翻案判据）。
⚠ **先立两侧对照**（第三十九轮 + 第五十七轮各踩过一次）：
阴性 = 同一个 page 连读两次应当 0 条差异；
阳性 = 人为在第二跑前改一处 DOM，必须被报出来。
**报不出阳性、或阴性就报几百条，都说明尺子量错了东西——先修尺子再读结论。**

### 1b. 走完之后：**进收尾判定**

用户 2026-09-20 拍的是 **B：走完已点名的名单再收**。名单走完（1a 做完）就该：

1. 把四条收工判据逐条量一遍写进交接（**别引用散文，现场量**）；
2. 把「已知取样面」那张名单标成走空；
3. 明确告诉用户：**继续下去只能靠造新面，产出率约一半，边际收益递减。**

⚠ **别自己顺手开新面**。方案 B 的边界就是这张名单。

### 1c. 两条没定性的小账（老的，仍未动；**不是必须做**）

| 场景 | 读数 |
| --- | --- |
| `agent-chat#completed-turn` | 本仓分栏格 `+0y+24x`，上游在分栏**根**上 `+0y+8x` |
| 37 个终态 | 上游分栏根 `+0y+8x`（基线 8x），本仓 splitpanes 根**不裁** |

要定性就直接**截图比**。⚠ 也完全可能判成「不是账」——
8px 的横向裁剪在两个不同构的分栏 primitive 之间，很可能只是实现字面。

### 1d. ⚠ 别去动的那一条（已判、已登记、三条改法都被读数否掉）

上游 `thread-list-pin#mobile-drawer` 的 **Export 子菜单在视口外**
（基线 x=[246,427] / 200% x=[201,545]，vw=375；本仓两档都完整可见）。
**已登记在 `viewport-fit.spec.ts` 的 `KNOWN_OUTSIDE` 里**，翻案判据在表旁边。
**要动它先读 open-accounts 第五十六轮第四节。**

### 1e. 一条**未验证**的线索（别当读数）

`motion-reduce` / `prefers-reduced-motion` 的**源文件数**上游 11 · 本仓 12。
源码计数，不是渲染读数；对照上下文已钉 `reducedMotion: "reduce"` 且台账 0。
**要当账先实测。**

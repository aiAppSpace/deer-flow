...
**上一轮（第五十三轮）是完整收工的**：`41ccb229` 已推送、全套门禁本机全绿、
...
1. **复核 `41ccb229` 的 CI**（第五十三轮推完时还在 `in_progress`）。
...
**名单早空了，第五十二/五十三两轮都是「造新面」。** 这一轮同样是造面。

⚠ **第五十三轮的 1a（断点自变量）已经做完了，0 条，别重做。**
读数：每应用 114 个「终态×断点」，本仓 18 处 / 上游 18 处，**18 个签名逐字相同**。

按「便宜且有先例」排序：

### 1a. 还两条门禁的登记表（**里面有一条上游单边的真账**）

`content-reachable.spec.ts` 的两张表都是**欠账，不是结清**：

```
纵向表（1 条，两边逐字相同）
  thread-list-pin#mobile-drawer   50px（基线）/ 100px（200%）

横向表（11 条，属 WCAG 1.4.10 Reflow）
  vue/react thread-list-pin#mobile-drawer  165x / 304x   两边逐字相同
  vue/react browser-feature                0y/217x       两边同值，层不同
  vue/react integrations ×3                0y/41x        两边逐字相同
  react/sidecar-chat                       0y/70x        ⚠ **只有上游**，本仓同处仅 13x
```

⚠ **`react/sidecar-chat` 是上游单边的真缺陷**，登记只是因为它属于横向那一族要整族一起还。
按用户那条「两边都不对时取业界做法**两边同改**」，两张表都该还。
还完把表清空；**清表之前先跑变异验证**，确认表空了之后门禁还会红。

⚠ 第五十三轮还量到断点轴上**另外 18 条两边共有的**（Sheet 头 `50y/265x`×6 @mobile
等，读数在 open-accounts 第五十三轮第一节）——它们**没有**进登记表，
因为门禁目前只跑每个终态自己那一档。还这一族的时候一并看。

### 1b. 造一个新面——**候选已经不多了，以下是还没量过的轴**

这两条是**建议**，不是判过的账。自己先当探针量一遍，**有收获再常驻**，
...
### 1c. 第五十二轮没定性的两条小的（门禁看不见，因为它们够得到）
...
- **已知取样面名单走空，且最后三轮开面无收获** ⬜ ← **只差这一条**
  ...
  连续无收获的轮次：1（第五十三轮）
    ...
    53  断点轴 375 / 768            0 条 ✅   ← 第 1 轮
  ```

  ⚠ 第五十三轮那个 0 **是有信息量的 0**（尺子在那两档上报得出 18 处，不是 0），
  而且它顺带掉出 5 条已判的行为差异——**开面有产出，只是这一轮的产出不是缺陷**。
...
```
<第五十三轮>  改了新门禁 + 守卫常量，**推完必须自己复核这条 run**
ade8f3f3  第五十二轮   verify + parity **双绿**（attempt 1，第五十三轮中途复核过）
...
**第五十三轮收工时这几条命令给出的是（拿它对照，不一致就先查为什么）：**

```
HEAD 41ccb229   工作树干净   未推送 0
...
⚠ 这几个数**第五十二、五十三两轮一个都没动**——那两轮改的是渲染、可达性与门禁，
**没有动取样面**。所以新窗口量出来应当与上面逐字相同；对不上就先查为什么，别往下做。

**本机门禁最近一次全绿的读数**（第五十三轮收工时，都是真跑出来的）：

```
verify      0   337 文件 / 2726 单测 · 词典 1144 key
make e2e    0   297 passed / 3.0m          ← 第五十二轮实测
e2e-parity  0   **210 passed / 48.0m**     ← 第五十三轮实测
                 52 轮 +2 content-reachable · 53 轮 +2 overlay-survives-resize
                 keyboard-order 双向那条仍是最慢的一支（约 10.5m）
```
...
e2e-parity 用例数                             210   （第五十三轮 +2 overlay-survives-resize；第五十二轮 +2 content-reachable）

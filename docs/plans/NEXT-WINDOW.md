...
**上一轮（第五十四轮）是完整收工的**：代码已推送、全套门禁本机全绿、
工作树干净。所以这一轮是**从「第一步」那一节开新面**，不是接半截活。
...
1. **复核第五十四轮那次推送的 CI**（推完时还在 `in_progress`）。
...
**名单早空了；第五十二/五十三两轮造新面，第五十四轮还账。** 这一轮回到造面。

⚠ **第五十四轮的 1a（还两张登记表）已经做完了，两张表现在都是空的，别重做。**
读数：12 行摊开是四处——3 行是尺子把 `p-4 sr-only` 当成了内容（已修尺子），
8 行是两边同值的真缺陷（`browser-feature` 217x / `integrations` 41x，已两边同改），
1 行是上游单边（sidecar 70x，已两边同改，**而查它时掉出本仓发送键出界 13px**）。
完整判词见 open-accounts 第五十四轮。

### 1a. 造一个新面——**首推这一条，因为第五十四轮已经拿它掉出过东西**

**「点得动吗」× 200% 文本。** 第三十八轮那条单应用不变量
（57 终态 ×2 应用，收紧后 0 条）**只在基线字号上跑过**。
第五十四轮拿它当一次性探针，在 200% 那一档当场报出：

```
browser-feature [200%]  两个应用逐字相同（基线那档 miss=0）
  <input> / "…" / "Close browser"   inViewport=false    ← 关闭键 x 起 1433，视口 1280
```

⚠ **它和 `content-reachable` 不重复**，这一点有读数：本仓 sidecar 发送键
200% 下 `x=[1229,1293]`、视口 1280（出界 13px），**`content-reachable` 看不见它**
——它不是「盒子裁自己」，是「把兄弟推出视口」，13px 也够不到 24 的门限。
**两把尺子量的是两件事。**

⚠ **仪器有一个已知的洞，先补再读结论**（第五十四轮实测）：
探针在 `integrations` 上**基线那档就报 48 条 miss**，全是设置对话框背后被遮住的
按钮——`elementFromPoint` 打到遮罩上。**那不是缺陷，是模态本来就该挡住背景。**
补法与第四十四轮 `aria-hidden` 那条同形：把「被模态遮住」排掉
（祖先有 `aria-hidden`/`inert`，或命中的是 overlay），再读剩下的。
⚠ 还有一类要排：主区 header 是 `absolute`，侧栏格会压在它上面，
两个应用**逐字相同**地各报 2 条（`Close` / `Export`）——同样不是缺陷。

**先拿已知答案的样本验仪器**（第三十九轮判词）：`browser-feature` 改前那三条
（`inViewport=false`）与改后（`fullyInView=true`）就是现成的双向样本。

### 1b. 还没量过的另外两条轴（**建议，不是判过的账**）

| 候选 | 为什么值得量 | ⚠ 先看 |
| --- | --- | --- |
| **`prefers-reduced-motion` 之外的用户偏好**：`forced-colors`（Windows 高对比）/ `prefers-contrast` | 两边的 primitive 不同构（Radix vs reka），强制配色下谁把 `border`/`outline` 画丢是量得出来的 | 对照上下文已经钉了 `reducedMotion: "reduce"`，别和它搞混 |
| **同一条流在两次进入之间的幂等**：同一个终态连跑两次，第二次的 aria/几何/请求与第一次比 | 第四十八轮那条飘就是「第二次不一样」，但那是跨应用比；**同应用两跑自比**还没做过 | 第五十轮判过「遍历本身可能改变被测对象」——自比要用**新开 page**，别复用 |

### 1c. 三条没定性的小账（两条老的 + 一条第五十四轮新记的）

| 场景 | 读数 | 状态 |
| --- | --- | --- |
| `agent-chat#completed-turn` | 本仓分栏格 `+0y+24x`，上游在分栏**根**上 `+0y+8x` | 第五十二轮记的，未定性 |
| 37 个终态 | 上游分栏根 `+0y+8x`（基线 8x），本仓 splitpanes 根**不裁** | 同上 |
| ~~两个 composer footer 的 `gap`~~ | ~~上游调用点 `gap-2`，本仓 scoped CSS `0.25rem`~~ | ✅ **第五十四轮当轮就量了并改掉**：200% 下 rowGap 本仓 8px / 上游 16px，footer 高 172 vs 180。两个调用点（sidecar + 主输入框）各补 `gap-2!` |

前两条要定性就直接**截图比**，别再堆新尺子。⚠ 也完全可能判成「不是账」——
8px 的横向裁剪在两个不同构的分栏 primitive 之间，很可能只是实现字面。

第三条已经结清，留在表里是因为它的**判词**值钱：那 4px 在换行之前
被 free space 吃掉、外沿逐像素相同，**台账因此报不出它**——
是第五十四轮先改了 sidecar 的换行，它才现形。

> **改布局会让此前藏着的差异现形，所以改完要重新量一遍整片，
> 而不是只量你打算改的那一处。**
...
```
<第五十四轮>  **两个应用的渲染都改了**（工具条换行 / 状态格重排 / sidecar 分组），
              **推完必须自己复核这条 run**——尤其是 parity，它跑的是台账本体
...
**第五十四轮收工时这几条命令给出的是（拿它对照，不一致就先查为什么）：**
...
⚠ 这几个数**第五十二、五十三、五十四三轮一个都没动**——那三轮改的是渲染、
...
```
verify      0   337 文件 / 2726 单测        ← 第五十四轮实测（改完 gap 之后又跑了一遍）
make e2e    0   297 passed / 1.9m           ← 第五十四轮实测
e2e-parity  0   **210 passed / 47.0m**      ← 第五十四轮实测（**跑了两遍**，见下）

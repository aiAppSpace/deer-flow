...
**上一轮（第五十六轮）是完整收工的**：代码已推送、全套门禁本机全绿、工作树干净。
那条链**已经修到底**，门禁也落地了——所以这一轮是**从「第一步」1a 开新面**。

⚠ **2026-09-20 用户拍板了收尾方案：走 B——「走完已点名的名单再收」。**
名单上只剩两个面（1a / 1b）。**走完就进收尾判定，不要再无限造新面。**
...
1. **复核第五十六轮那次推送的 CI**（推完时还在 `in_progress`）。
...
**第五十六轮把「popper 比视口宽」那条链修到底了，门禁也落地了。**
按 2026-09-20 用户拍板的**方案 B**（走完已点名的名单再收），
这一轮开**名单上剩下的两个面之一**。名单走完这个项目就到收尾判定了。

### 1a. 开 `forced-colors` / `prefers-contrast` 这个面（**首推**）

两边 primitive 不同构（Radix vs reka），**强制配色下谁把 `border`/`outline` 画丢
是量得出来的**——这类缺陷在普通配色下完全看不见，台账也报不出。

做法：`browser.newContext({ forcedColors: "active" })` / `{ contrast: "more" }`，
其余照 `viewport-fit.spec.ts` 的骨架（那份是这一族最新、自检最全的一个模板）。

⚠ **对照上下文已经钉了 `reducedMotion: "reduce"`**（见 `support/context-options.ts`），
别和它搞混，也别顺手把它改掉。
⚠ **先拿一个已知答案的样本验仪器**（第三十九轮判词）：
`forcedColors: "active"` 下**所有背景色都会被系统色替换**，
所以「两边颜色都变了」是对照成立的信号，不是缺陷；
要比的是**谁丢了边框/轮廓**（`outline` 在强制配色下是唯一可靠的边界表达）。

### 1b. 另一个面：同一条流两次进入之间的幂等

第四十八轮那条飘是「第二次不一样」，但那是**跨应用**比；
**同应用两跑自比**还没做过。⚠ 自比要用**新开 page**
（第五十轮判过：遍历本身可能改变被测对象）。
...
**已登记在 `viewport-fit.spec.ts` 的 `KNOWN_OUTSIDE` 里**，翻案判据写在表旁边。
试过并被读数否掉的三条：`sticky="always"` / 给 SubContent 加 `max-w-(available)`
（**会让基线退化**）/ 把本仓子菜单改成 portal（不能翻 wave 95 的判词）。
**要动它，先读 open-accounts 第五十六轮第四节。**

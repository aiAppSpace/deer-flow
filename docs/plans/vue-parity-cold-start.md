...
**接手时的状态（2026-09-12 第九轮收工实测，不是估计）**：

- 工作区干净；**本地领先 `origin/main-wc` 三百多个提交、全部未推送**
  （没有收到过推送指令；要推就先问用户）。
- 对照台账 **159 唯一行 / 179 多重集 / 138 个场景-维度**。
...
```bash
# 2026-09-12 第九轮收工实测（每一条都是那一轮真跑出来的，不是抄的；
# 第九轮只动 scenarios.ts + 文档（台账一个字节没动），按节奏只跑 verify 与 e2e-parity，
# 其余读数是第五 / 七轮真跑的，第五轮九条全绿）
make -C <abs>/frontend-vue verify         # exit 0；321 文件 / **2642** 单测；词典 1139 key / 15 unused；产品 SFC 267
make -C <abs>/frontend-vue e2e-parity     # **146 passed**（整条 16.6 分钟）
...
## 上一轮（2026-09-12 第十轮）做了什么，下一轮从哪接

**给几何档加 `borderRadius`，第一次整套跑就抓到一处形状差异并当轮修掉。**

第九轮判 `ui/input-group` 不移植时写下了那个结论的边界（几何档不量形状），
这一轮把那一档补上。加之前过了坑 258 那一问（**有没有一种变异能让它响、
而现有的档都不响**），答得上来。**新尺子先量自己**：两条单测钉住
「只改圆角时它响而其余各档一条都不响」与「圆角相同不报」。

第一次整套跑 **5 行，全部同一处**（`borderRadius React=3.35544e+07px Vue=8px`，
前者是 Tailwind v4 的 `rounded-full`）：**噪声 0 行、与别的档重复 0 行**。
根因是 `ProcessingToolStep.vue` **手抄了 `ui/badge` 的外观**（本仓明明有 Badge、
基类还与上游逐字相同），三处换成 Badge 之后 5 行 → 0 行。

**下一轮最该先拿的（按顺序）**：

1. **把「手抄 primitive 外观」这一族全仓扫一遍。** 这一轮那处不是孤例——
   同一形状此前已经出现过至少三次（`handwritten-input` 那一轮的三份本地常量、
   wave 141 逐个对比基类时的 10 处、这一轮的 Badge）。
   **现在有新工具了**：`borderRadius` 这一档能看见「形状抄错」，而 wave 141 那份
   基类比对守卫（`primitive-class-overrides` / 基类清单）看的是**调用点传的 class**，
   看不见「压根没用 primitive、自己手写一个」。
   判据用 `handwritten-button` / `handwritten-input` 那两份守卫的做法：
   **扫出所有「长得像某个 primitive 的手写元素」，逐条对上游、该改的改、剩下的进双向清单。**
   起手式：grep `bg-secondary text-secondary-foreground`、`bg-primary text-primary-foreground`
   这类**变体色对**出现在非 `ui/` 文件里的地方。
2. **方向 C（把散文里的断言变成守卫）**：下面那一节列着还没筛的几条。
   第四到第十轮这条一直没动过，而它在 wave 101~106 连着五轮都有货。
3. **继续开维度**——边际收益在下降，挑之前先答一句「这一屏上有什么东西会挤出去 /
   会在深色下现形 / 形状会不会不一样」。还没开过窄屏的整页只剩 `agents` 画廊。

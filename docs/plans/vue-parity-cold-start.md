...
make -C <abs>/frontend-vue e2e-mock       # 319 passed（274 + 22 + 15 + 2 + 6）；**第二十一轮真跑**
                                          ...
                                          #    实际隔了 7 轮，CI 上那条 375px 的红因此躺了 6 天）
...
1. **把「批不超过 4 轮」变成机器判据。** 这条规则写在上面的节奏块里，而
   `e2e-mock` 上一次真跑是**第十三轮**——到第二十轮隔了 **7 轮**，将近限额两倍，
   CI 上那条红因此躺了 6 天。**又一条「写下来了、没被执行」的规则**
   （同管道退出码那条）。可行做法：签入 `baseline/gate-runs.json` 记录每条全套
   门禁上次实跑的提交，守卫比 `git rev-list --count <那个提交>..HEAD`，超阈值就红。
   **要单独一轮做足负向验证**——仓促做出来的守卫正是本仓反复吃亏的东西。

...
**接手时的状态（2026-09-12 第六轮收工实测，不是估计）**：

- 工作区干净；**本地领先 `origin/main-wc` 三百多个提交、全部未推送**
  （没有收到过推送指令；要推就先问用户）。
- 对照台账 **162 唯一行 / 182 多重集 / 137 个场景-维度**。
...
**「台账还剩多少行」已经不能当坐标系了。** 第六轮收工时 **162 行逐条都有判词**，
而且分布极不均匀：**99 行**是 `div[scroll-area-viewport]` 那一笔判过的账
（含它在 `tabOrder` 上的投影）、**20 行**是请求层那两族（`retry` 与抽屉挂载拓扑），
...
```bash
# 2026-09-12 第六轮收工实测（每一条都是那一轮真跑出来的，不是抄的；
# 第六轮只动 scenarios.ts + baseline + 文档，按节奏只跑 verify 与 e2e-parity，
# 其余读数是第五轮真跑的，那一轮九条全绿）
...
## 上一轮（2026-09-12 第六轮）做了什么，下一轮从哪接

**这一轮的货是一条否定结论、一次判词订正，和两个新维度的永久覆盖。**
台账 154 → 162 唯一行、133 → 137 场景-维度，**新增 8 行全部属于两条判过的账**。

1. **把第五轮那条判据用到底、然后宣布它见底。** 判据是「『台账 0 行』先问
...
2. **`scheduled-tasks` 补 `mobile` 与 `dark` 两维，几何与 aria 各零行。**
   ...
   是从现在起有机器守着它在 375px 与深色下的排布。
3. **顺手推翻了自己一句判词**：`thread-list-pin#mobile-drawer` 那条写着
   「本仓做的网络工作严格更少」——**只在抽屉被打开过时成立**。抽屉没打开时
   （手机常态）方向相反，本仓多发三次请求。判词就地订正成「两种取舍：
   本仓预取、上游按需」，仍判不跟。

**下一轮最该先拿的（按顺序）**：

1. **`chat-thread-init-ordering` 那 3 行 `button "Edit and rerun"`。**
   **这是台账上最后一条判词里还带「先怀疑」的行**，backlog 写着
   「先加临时 dump 看读数，别猜」，而它一直没人做。
2. **继续给「整页」形状补窄屏。** 第六轮证明这条还有覆盖面可开，只是这一页恰好是对的。
   还没被窄屏看过的整页：`project-detail`、`agents` 画廊（`agents-feature-disabled`）、
   `thread-history`（聊天页那一族由 `chat` 跑满矩阵，可跳过）。

...
**接手时的状态（2026-09-12 第十七轮收工实测，不是估计）**：
...
```bash
# 2026-09-12 第十七轮收工实测（每一条都是真跑出来的，不是抄的；
# 第十七轮动了 ThreadSidebar 的 footer 按钮，所以 verify / e2e-mock / e2e-parity 都真跑了；
# e2e-visual / icon-parity 是第十三轮真跑的；
# 其余读数是第五 / 七轮真跑的，第五轮九条全绿）
make -C <abs>/frontend-vue verify         # exit 0；**329 文件 / 2669** 单测；词典 1139 key / 15 unused
...
## 上一轮（2026-09-12 第十七轮）做了什么，下一轮从哪接

**修了第十二轮那四颗按钮的第五个兄弟；更值钱的是撞到「负向验证自己也会是空操作」。**

把 78 条负向断言按「有没有历史标记」再筛一次，只留还在当现在时用的（29 条）。
出货两处：① `icon-parity-tool.test.ts` 有一条断言**钉住了一句仓库已经宣布作废的话**
（`icon-parity.mjs` 的文件头写明那句话从 wave 111 起是假的），尺子换成真正要守的三样；
② 回扫「状态条件类」（第十二轮的账）时，静态对比发现 **footer 那颗设置键
35 个 cva token 里只有 12 个**——漏掉的活项包括**键盘焦点环**。
那是第十二轮那四颗导航键的第五个兄弟，当时跳过它是因为 wave 74 的豁免，
而**那条豁免说的是 `peer/menu-button` 与收起态尺寸，不是焦点环——范围被读宽了**。

**N1 第一次跑绿了**，查下去是**变异根本没生效**（`.replace(x, "", 1)` 命中了文件里
更早的另一处）。改成「先定位到元素、打印改动前后、再断言」之后当场红。

**下一轮最该先拿的（按顺序）**：

1. **给 todos 造夹具、挂进取样面，再改 TodoList 的容器层**（第十三轮的账，一直没动）。
   起手式：现有 `PARITY_SCENARIOS` 里**没有任何 `values.todos`**；
   容器层要改的是定高 `h-28` ＋ 高度过渡 ＋ ScrollArea ＋ 条目 `hover:bg-muted`，
   会动到折叠动画——**没有读数不要动**。
2. **把「豁免的范围被读宽了」当成一条新的筛法**：仓里每一条 `ALLOWED` / `INERT` /
   豁免表条目，都写着「为什么这一处例外」。**去问那条理由覆盖的是不是整条**——
   第十七轮这颗按钮就是「豁免说的是 A，而被当成了 A+B+C」。
   起手式：`grep -rn 'ALLOWED\|INERT\|豁免' frontend-vue/tests/guards`，
   逐条读理由，再去看被豁免的对象**实际偏离了多少**。
3. **方向 C 继续**（还剩几族 token，见下面那一节）。

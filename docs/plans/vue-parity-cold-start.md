...
```bash
# 2026-09-12 第十八轮收工实测（每一条都是真跑出来的，不是抄的；
# 第十八轮新接了一屏进取样面并改了 TodoList，verify / e2e-parity 各跑了三遍；
# e2e-visual / icon-parity 是第十三轮真跑的；
# 其余读数是第五 / 七轮真跑的，第五轮九条全绿）
make -C <abs>/frontend-vue verify         # exit 0；**330 文件 / 2672** 单测；词典 1140 key / 15 unused
...
## 上一轮（2026-09-12 第十八轮）做了什么，下一轮从哪接

**结清第十三轮挂的那笔账：给 todos 造夹具、挂进取样面、量到读数再改 TodoList。
整个循环走完——挂锚点 → 22 行 → 逐行追到根因 → 照上游补 → 归零。**

沿途挖出五处，全是「这一屏没进过取样面所以谁也说不出来」的那类：
① mock 从来不吐 `values.todos`（**任何 `backend: "mock"` 的用例都看不见这一屏**）；
② `/history` 手抄的第三份 channel values 已漂——**同一个字段两种行为**
（本仓读 `/state` 看得见、上游用 `useStream` 从 `/history` 水合看不见），已做成门禁；
③ 待办面板**一层定位包裹都没有**，欢迎态下被 Welcome 块压住、点不动；
④ 标题文字没单独一层 → `x Δ-24 / width Δ+24`（正好是图标 16 + gap 8）；
⑤ `<li>` 缺 `QueueItem` 的 `px-3 py-1` → `x Δ-12 / width Δ+24`、行距差 8。

**下一轮最该先拿的（按顺序）**：

1. **把「豁免的范围被读宽了」当筛法过一遍所有 ALLOWED / INERT 表**
   （第十七轮留的，没动）。起手式：
   `grep -rn 'ALLOWED\|INERT\|豁免' frontend-vue/tests/guards`，
   逐条读理由，再去看被豁免的对象**实际偏离了多少**——
   第十七轮那颗 footer 按钮就是「豁免说的是 A，而被当成了 A+B+C」。

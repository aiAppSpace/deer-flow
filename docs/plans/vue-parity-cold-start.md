...
- 对照台账 **0 唯一行 / 0 多重集 / 162 个场景-维度**。
  ...
  **但 0 不等于对齐完成**：台账只覆盖这 162 个场景-维度，取样面之外它一个字都没说。
...
1. **把取样面往没人看过的维度开一扇窗。** 2026-09-11 实测：
   ...
   - **162 个场景-维度里仍有 138 个是 desktop**；非 desktop 的 9 族逐字是：
...
#   跑一次即可，别照抄。
make -C <abs>/frontend-vue verify         # exit 0；**332 文件 / 2700** 单测；词典 1143 key / 15 unused
make -C <abs>/frontend-vue e2e-parity     # **173 passed**（第三十八轮实测 18.1 分钟；
                                          #   耗时本来就没人守，机况一变就漂）
                                          ...
                                          #  台账 0 唯一行 / 0 多重集 / 162 场景-维度（第三十八轮 accept）
                                          #  （第十二、十三两轮基线文件都一个字节没动；

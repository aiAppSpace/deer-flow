...
- 对照台账 **0 唯一行 / 0 多重集 / 161 个场景-维度**。
  ...
  **但 0 不等于对齐完成**：台账只覆盖这 161 个场景-维度，取样面之外它一个字都没说。
...
1. **把取样面往没人看过的维度开一扇窗。** 2026-09-11 实测：
   ...
   - **161 个场景-维度里仍有 138 个是 desktop**；非 desktop 的 8 族逐字是：
...
make -C <abs>/frontend-vue e2e-parity     # **172 passed**（第三十八轮实测 18.1 分钟；
                                          #   耗时本来就没人守，机况一变就漂）
                                          ...
                                          #  台账 0 唯一行 / 0 多重集 / 161 场景-维度（第三十八轮 accept）
                                          #  （第十二、十三两轮基线文件都一个字节没动；

...
- 工作区干净；**已推送**——2026-09-15 收到推送指令后把 `main-wc` 推到
  ...
  要推先问用户。
- 对照台账 **159 唯一行 / 179 多重集 / 140 个场景-维度**（第十八轮新增两条
...
1. **把取样面往没人看过的维度开一扇窗。** 2026-09-11 实测：
   ...
   - **140 个场景-维度里仍有 118 个是 desktop**；非 desktop 的 7 族逐字是：
...
make -C <abs>/frontend-vue e2e-parity     # **148 passed**（整条 17.3 分钟）
                                          #  台账 159 唯一行 / 179 多重集 / 140 场景-维度
                                          #  （第十二、十三两轮基线文件都一个字节没动；
...
1. **收起态的侧栏整个不在取样面。** 上游收起靠 CSS（节点还在：分组标题
   `-mt-8 opacity-0`、菜单动作 `hidden`、菜单键 `size-8! p-2!`），
   本仓靠 `v-if="sidebarExpanded"`（**节点直接删掉**）。
   `opacity-0` 不会把节点移出可访问性树，所以**上游收起后仍会朗读分组标题、
   本仓不会**——这很可能是上游的缺陷（不可见内容仍被朗读），
   真要动就是两边同改。**起手式照第十八轮那条纪律**：先确认
   `SidebarTrigger` 在 workspace 外壳里渲染得出来、点得到，再谈挂场景。
2. **确认 CI 那条修复真的绿了。** 本轮照 `external-gates` 逐字补齐了

...
## 上一轮（2026-09-12 第九轮）做了什么，下一轮从哪接

**backlog 上那笔 `ui/input-group` 判「不移植」——不是靠论证，是靠先造一个读数。**

工单自己写着「动它之前先确认台账上它现在是 0 行」。确认了，然后把这句话反过来用：
**既然没有读数在推动，那就先把它接进取样面，再决定要不要动。**

核实之后发现本仓那块外壳是**量着抄的**：上游调用点用
`*:data-[slot='input-group']:rounded-2xl` 把 InputGroup 顶成 `rounded-2xl`，
再叠 `bg-background/85 backdrop-blur-sm z-10`，本仓 `ComposerSurface.vue` 逐条对得上。

产出：把 `[data-slot="input-group"]`（两边共有的**结构坐标**）挂到**跑满 12 维**的
`chat` 上。**12 维零新增行**，且这个 0 是算出来的（探针：每屏只匹配一份、
样本非空、两边逐位相同）。判词**不移植**，边界与两条翻案判据写在挂账清单里。

**下一轮最该先拿的（按顺序）**：

1. **给几何档加 `borderRadius`（方向 B）。** 这一轮撞出来的缺口：几何档量
   位置/尺寸/前景色/背景色/字号/字重/opacity/命中/伪元素，**不量形状**。
   **坑 258 那句「有没有一种变异能让它响、而现有的档都不响」这次答得上来**：
   把 composer 的 `rounded-2xl` 改成 `rounded-md`，现有各档**一条都不响**。
   加之前照 wave 94/96 的规矩先问两句：其中几行是它自己造的、几行是别的档已经报过的。
   **代价要先想清楚**：这一档会落在**每一个**锚点上，第一次 accept 大概率一批新行。
2. **方向 C（把散文里的断言变成守卫）**：下面那一节列着还没筛的几条。
   第四到第九轮这条一直没动过，而它在 wave 101~106 连着五轮都有货。
3. **继续开维度**——**边际收益在下降**：第六、八、九三轮新开的维度/锚点全是 0 行。
   挑之前先答一句「这一屏上有什么东西会挤出去 / 会在深色下现形」，
   答不上来就是在碰运气。还没开过窄屏的整页只剩 `agents` 画廊。

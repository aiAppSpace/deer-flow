...
## 接手时的状态快照（**不是**开工指令——开工指令在 `NEXT-WINDOW.md`）

⚠ 这一节 2026-09-21 之前的标题是「开工指令（整段贴给新窗口）」。
**改掉是因为它名不副实**：这一节没有「这一轮干什么」，也没有开工前置三件事
（复核 CI / 读「别再做的事」/ 读「已经量过」），把它整段贴给新窗口
会得到一个**没有本轮任务**的背景块。本轮任务只在 `NEXT-WINDOW.md`。
...
2. `docs/plans/vue-parity-open-accounts.md` —— 挂账总清单，**7459 行 / 520K**
   ...
   ⚠ **轮次顺序是乱的，`tail` 找不到最新一轮**（2026-09-23 实测：
   带轮次的 `##` 小节 21 条，出现顺序是 38→43 升序，然后 **58→44 降序**；
   **最大的第五十八轮在文件中段，文件末尾是第四十四轮**）。
   要定位最新一轮，用
   `grep -nE '^## .*第[〇一二三四五六七八九十]+轮' docs/plans/vue-parity-open-accounts.md`
   自己挑轮次号最大的那条。开工不用整篇读：
...
- **每轮收工写 `NEXT-WINDOW.md` + `vue-parity-open-accounts.md` + 记忆，然后自动开下一轮**，
...
6. 提交（fix / chore / docs 分开），更新 `NEXT-WINDOW.md` + `vue-parity-open-accounts.md` + 记忆。
7. **直接开下一轮。**
...
**完整命令就在下面这一段里**，不必去别处找。实操上不必自己 `next build` +
...
（React 那三条写在上面的门禁块里，读数与 3002 端口的跑法都以那里为准。）
...
- **名单里有一条与其余几条不同类**（wave 102 加的；⚠ **条数以上面那条
  「从七条减到六条」为准，这里不再重复写数——一个数只留一个出处**）：
...
## 上一轮（2026-09-17 第三十六轮）做了什么

**台账推到 0（起点 170 个投影）。** 三件事：`stream_mode` 拔掉 `values`
（连带 `stream_resumable`）、补上 `useThreadMetadata` 这一层、
两边同改去掉 `context.thread_id`。

**两条教训写在 `NEXT-WINDOW.md` 里，这里只留一句**：上一轮判成「架构差异、退让」
的那条，这一轮翻案了——`thread-title-sync` 不是「本仓不需要第二个查询」，
而是**本仓缺了那一层**：四处代码都在失效 `["thread","metadata",id]`，
却没有任何查询拥有这个 key，四处全是空操作。
**「本仓不需要」和「本仓缺了那一层」长得很像。**
...
> **⚠ 第三十六轮把对照台账推到 0（起点 170 个投影）。** 这张单子因此重排了：
...
2. **扩取样面——台账清零之后，新差异只能从这里来。**
   ...
   先看 `baseline/parity-route-sampling.json` 哪些路由取样点最少；
   对照场景 id **就是上游 spec 文件名**，想不出对应 spec 就加不了新场景；
   加新场景必须配终态断言，否则「零差异」可能只是「压根没采到」。

3. **三张 pending 表是工单队列，不是可以停住的账**
   （记忆 `deerflow-upstream-features-must-land-in-vue`）。
   台账看不见「本仓整个屏都没做」这一类——它只比两边都到得了的屏。

> **覆盖率棘轮现状**：covered 37 / pending 1 / exempt 3。
> 那 1 条 pending（`artifact-table-performance`）**已经有完整判词，别再重新问**——
...
> **⚠ 这一段是历史，留着看判据怎么演进。**
> 里面点名的账 F（Mermaid 工具条）与账 G（`channels#settings-panel-connected`）
> **第二十三轮就结清了**，台账第三十六轮清零。
...
- **「量不出差异」的准确含义是「这些取样点上量不出」**，不是「两个应用一样」。
  ...
  「天生看不见的那几类」原文在**已删的** `vue-parity-handoff.md` 里
  （节名「台账天生看不见的八类差异」）；取回：
  `git show 52f61c0d:docs/plans/vue-parity-handoff.md`，**别 checkout 回工作树**。
  ⚠ 本文第 1093 行自己写的是「现在少了两类半」，和这里的「八类」不是同一时点的数
  ——**两处都别当现状**。

...
**接手时的状态（2026-09-12 第八轮收工实测，不是估计）**：
...
**「台账还剩多少行」已经不能当坐标系了。** 第八轮收工时 **159 行逐条都有判词**，
...
```bash
# 2026-09-12 第八轮收工实测（每一条都是那一轮真跑出来的，不是抄的；
# 第八轮只动 scenarios.ts + baseline + 文档，按节奏只跑 verify 与 e2e-parity，
# 其余读数是第五 / 七轮真跑的，第五轮九条全绿）
make -C <abs>/frontend-vue verify         # exit 0；321 文件 / **2642** 单测；词典 1139 key / 15 unused；产品 SFC 267
make -C <abs>/frontend-vue e2e-parity     # **146 passed**（整条 16.9 分钟）
...
## 上一轮（2026-09-12 第八轮）做了什么，下一轮从哪接

**给 `project-detail` 补窄屏（第二个「整页」样本），三维全 0 行；
过程里踩出一个新形状的锚点陷阱，写成了一条补充判据。**

这一页原来的两个锚点都是夹具里的文字，几何档只量得到两个文本节点，
...
（都是 `New chat`），**中文下不是**（`新对话` vs `新建对话`）。那条跨语言正则
因此在 en-US 桌面上匹配到两份，`.first()` 量的是侧栏那颗。
换成按 href 的结构锚点之后三维全 0。

**下一轮最该先拿的（按顺序）**：

1. **`ui/input-group` 没有移植**：三个 composer（`ChatComposer` /
...
2. **继续开维度**，但**这条方向的边际收益在下降**：第六、八两轮各给一个整页补窄屏，
   两次都是 0 行。**下一次挑之前先答一句「这一页在 375px 下有什么东西会挤出去」**
   ——答不上来就说明是在碰运气，换方向 C 更划算。
   还没开过窄屏的整页只剩 `agents` 画廊（`agents-feature-disabled`）。
3. **方向 C（把散文里的断言变成守卫）**：下面那一节列着还没筛的几条。
   第四到第八轮这条一直没动过，而它在 wave 101~106 连着五轮都有货。

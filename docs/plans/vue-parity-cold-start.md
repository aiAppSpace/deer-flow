...
- 对照台账 **0 唯一行 / 0 多重集 / 147 个场景-维度**。
  ...
  **但 0 不等于对齐完成**：台账只覆盖这 147 个场景-维度，取样面之外它一个字都没说。
...
make -C <abs>/frontend-vue e2e-parity     # **158 passed**（第三十三轮实测 15.7 分钟；
                                          #   此前记的 17.3 分钟是旧机况，耗时本来就没人守）
                                          #  （156 → 158 是第三十七轮加的 animation-settle.spec.ts）
                                          #  `make parity-accept` 只跑 diff.spec.ts（3 passed，
                                          #   第二十三轮实测 13.1 分钟——它一条用例里抓全部场景）
                                          #  台账 0 唯一行 / 0 多重集 / 147 场景-维度（第三十六轮 accept）
                                          #  （第十二、十三两轮基线文件都一个字节没动；

...
**接手时的状态（2026-09-12 第十六轮收工实测，不是估计）**：

- 工作区干净；**本地领先 `origin/main-wc` 三百多个提交、全部未推送**
  （没有收到过推送指令；要推就先问用户）。
- 对照台账 **159 唯一行 / 179 多重集 / 138 个场景-维度**（第十二轮新增的
  4 行当轮修完归零，基线一个字节没动）。
...
```bash
# 2026-09-12 第十六轮收工实测（每一条都是真跑出来的，不是抄的；
# 第十六轮**没有改任何产品行为**（注释 + 一道门），只跑 verify；
# e2e-mock / e2e-parity 是第十五轮真跑的；
# e2e-visual / icon-parity 是第十三轮真跑的；
# 其余读数是第五 / 七轮真跑的，第五轮九条全绿）
make -C <abs>/frontend-vue verify         # exit 0；**329 文件 / 2668** 单测；词典 1139 key / 15 unused
...
## 上一轮（2026-09-12 第十六轮）做了什么，下一轮从哪接

**按 token 聚类剩下的几族逐条核，全部确有指名用例——出货的是「核」这个动作本身。**

`message.contract.ts` 的文件头说「这个文件之所以在 `app/` 而不是 `tests/`，
是因为 **`tests/` 根本不过 vue-tsc**」。实测：在 `tests/` 里塞
`const x: number = "s"`，**当场报 TS2322**。git 一查——`make typecheck-tests`
**2026-09-11 就接进了 `verify`**，而那份文件最后一次改动是 08-25。

这句话被当现在时事实用在**七个地方**，每一处都在支撑一个设计决定
（为什么住 `app/`、为什么不写类型断言、「没有任何机器说过话」）。
七处全部改成过去时并写明被哪条命令推翻，靠它支撑的结论逐条重判。
**其中一处是新守卫自己扫出来的——我手写的 grep 漏了它。**

同形的第二处：`icon-parity.mjs` 的文件头**专门**标注过「『不进任何门禁』那句话
从 wave 111 起就是假的」，而 `cross-app-by-design.mjs` 的登记里原样留着同一句
——**改在了发现它的地方，没改到另一处**（第十二、十三轮同一形状的第三次）。

新门 `stale-coverage-claims`：判据不是「不许写这句话」，而是「写了就得**现在**还成立」，
而「还成不成立」由 `verify` 的先决条件**算出来**；摘掉 `typecheck-tests`，门自己让路。
负向验证 3 条。

**下一轮最该先拿的（按顺序）**：

1. **把 `stale-coverage-claims` 的表再加两条。** 这一轮只进了一条
   （「tests/ 不过 typecheck」）。候选就在那次 78 条的扫描结果里——
   判据是**「这句断言还成不成立，能不能算出来」**，算不出来的不要进表。
   起手式：
   ```bash
   grep -rnE '没有任何(机器|门禁|人)|没人(守|看|管|钉)|不进任何门禁|空操作' frontend-vue/{app,tests,scripts,packages}
   ```
   逐条问「它依赖的那个事实，今天还是那样吗」——**这一轮两处都是「曾经对、后来假」**。
2. **给 todos 造夹具、挂进取样面，再改 TodoList 的容器层**（第十三轮的账）。
3. **把「正负相消」回扫现有取样面**（第十二轮的账）。

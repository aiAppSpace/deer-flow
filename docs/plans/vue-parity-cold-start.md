...
1. **族 A：`div[scroll-area-viewport]` 两边可 tab 性不同——一处根因占全部投影的 62%。**
   6 条差异条目 / **105 个投影**。React 的滚动视口能用 Tab 走到
   （Radix 在内容可滚动时给 viewport `tabindex=0`），本仓走不到。
   **这是交互逻辑上的真差异**：键盘用户在 React 能 Tab 进滚动区再用方向键翻。
   历轮把它记成「已判过的账」而没还——**按最终目标它必须还**。
   起手式：查 Radix `ScrollArea` 给 viewport 加 `tabindex` 的条件，
   对一遍 reka 的 `ScrollAreaViewport`，再决定是包装层补还是提 reka。

2. **族 D：多账号绑定块——唯一一处「功能」层面的不一致。**
   本仓的渠道设置页能列出同一 provider 下的多个账号、逐个断开、再添加，
   上游 `connectionByProvider` 把列表收敛成一条。
   **后端 `/api/channels/connections` 本来就返回列表**，所以这是上游丢信息
   ——走**两边同改**（给上游补），不是把本仓退回单账号。

3. **`chat-thread-init-ordering` 上 `context.thread_id` 两边不同**（第三十轮新尺子报出来的，
...
4. **把 `e2e-parity` 接进 CI——这把尺子从来没被这台笔记本以外的机器跑过。**
...
5. **方向 1：把取样面往没人看过的维度开一扇窗**（本文件开头那节排第一的一条）。
...
6. **账 J：后端的分支接口不继承 `agent_name`**（第二十九轮量出来的，见挂账清单）。
...
7. **方向 C 继续**（把「写下来当规则用、却没人守」的话变成守卫，判据见下面 C 节）。
...
8. ~~把「批不超过 4 轮」变成机器判据~~ —— **已降级**，前提被推翻，理由见下面

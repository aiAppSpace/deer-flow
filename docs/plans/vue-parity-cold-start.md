...
分支 `main-wc`，**接手时 HEAD 是 wave 146 的 docs 提交，已推到
...
- **台账的规则现在是「新出现、还没定过的行只能减不能增」**，不再是「保持 0」。
  `frontend-vue/baseline/parity-diff.json` 当前 **199 行 / 89 样本**
...
make -C <abs>/frontend-vue e2e-parity      # 97 passed（--list 报 97 tests in 3 files；旧读数记的 96 漏了 topology.spec.ts 一条）；台账 199 行 / 89 样本

...
- 对照台账 **146 唯一行 / 168 多重集 / 146 个场景-维度**。
...
1. **把取样面往没人看过的维度开一扇窗。** 2026-09-11 实测：
   ...
   - **146 个场景-维度里仍有 123 个是 desktop**；非 desktop 的 8 族逐字是：
...
make -C <abs>/frontend-vue e2e-parity     # **155 passed**（第二十九轮实测；第二十七轮整条 17.5 分钟；
                                          #   此前记的 17.3 分钟是旧机况，耗时本来就没人守）
                                          #  `make parity-accept` 只跑 diff.spec.ts（3 passed，
                                          #   第二十三轮实测 13.1 分钟——它一条用例里抓全部场景）
                                          #  台账 146 唯一行 / 168 多重集 / 146 场景-维度
                                          #  （第十二、十三两轮基线文件都一个字节没动；
...
1. **沿着账 C 那条线索：本仓 80 处 `:title=`，逐个问「上游这一处是什么」。**
   第二十四轮只改了侧栏那四颗，第二十九轮数了一遍：
   `grep -rn ':title=' frontend-vue/app` **80 处**（含注释里提到的，要先剥）。
   **判据是「两个应用给出同一种提示机制」，不是「有没有提示」**——
   `title` 是原生气泡、上游多数地方用的是 Radix Tooltip，两者在可访问性树上
   也不是一回事（tooltip 挂 `aria-describedby`，title 会变成可访问名的兜底）。
   **第二十四轮那条教训必须带着**：摘 `title` 之前先量「它原来还撑着什么」——
   那次摘掉之后三颗导航键从可访问性树上整个消失了，而 lint/typecheck/单测全绿。

2. ~~上游的 agent 会话页根本不传 `canBranch` / `onBranchTurn`~~ —— **第二十九轮已结清**，
...
2. **方向 1：把取样面往没人看过的维度开一扇窗**（本文件开头那节排第一的一条）。
   **146 个场景-维度里仍有 123 个是 desktop**，非 desktop 只有 8 族。
...
3. **账 J：后端的分支接口不继承 `agent_name`**（第二十九轮量出来的，见挂账清单）。
...
5. ~~把「批不超过 4 轮」变成机器判据~~ —— **已降级**，前提被推翻，理由见下面

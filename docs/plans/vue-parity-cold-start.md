...
1. **族 B：请求集合——两边发的接口不一样，43 个投影、9 条差异条目。**
   现在是最大的一族。**逐条分，不要当成一件事**：
   - `GET /api/integrations/lark/status`（12）与 `GET /api/scheduled-tasks`（12）
     ——**上游在轮询、本仓不发**。先量「本仓靠什么替代」（可能是 SSE 推送或压根没实现），
     判词取「同一个信息两边多久刷新一次」，不是「少发就是好」。
   - 产物那条 9 个投影是**上游失败后重试 3 次**（wave 128 判过是上游缺陷）
     ——按「两边同改」修上游那条重试。
   - `features` / `threads/search` 两向都有（各 1~2），**先查是不是时序**
     （两边都发，只是取样窗口切在中间），那是假差异。

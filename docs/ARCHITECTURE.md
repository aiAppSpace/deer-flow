...
skills/tools** — all isolated per conversation thread. The default frontend is a Next.js chat UI, with a coexisting Nuxt/Vue one on the secondary hostname;
...
- **Config system** — lives at repo root: `config.yaml` (models, tools, sandbox,
  ...
  **tracked in this repo**, seeded from the `*.example.*` templates when missing, and
  editable at runtime via the Gateway API. Operator-controlled third-party `plugins:` live only in `config.yaml`

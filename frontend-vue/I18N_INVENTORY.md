...
不是手工维护的组件 allowlist。当前 checkout 共有 253 个 Vue SFC：

- `app/app.vue`、`app/components/**/*.vue`、`app/layouts/**/*.vue`、
  `app/pages/**/*.vue` 中 251 个产品 SFC 全部进入 AST source guard；
...
| 类别               | 处理规则                                                                                                       | 当前结果                                 |
| ------------------ | -------------------------------------------------------------------------------------------------------------- | ---------------------------------------- |
| 用户可见产品文案   | 模板文本与 UI 脚本值必须来自唯一 `$i18n` owner                                                                 | 251 个产品 SFC 无核心英文硬编码          |
...
`en-US` 和 `zh-CN` 当前各有 1041 个完全一致的 leaf key。
